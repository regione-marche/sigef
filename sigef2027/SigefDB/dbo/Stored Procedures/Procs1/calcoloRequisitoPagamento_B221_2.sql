-- SALDO
--=== Data di ultimazione degli interventi programmati (entro 12 mesi dal ricevimento dell`atto di concessione, più eventuali proroghe) ===--
...
create PROCEDURE [dbo].[calcoloRequisitoPagamento_B221_2]
@ID_DOMANDA_PAGAMENTO int
AS
BEGIN

DECLARE @ID_PROGETTO INT, @CODICE INT,@RESULT INT,@PROROGA_3 INT,
		@DATA_FINE_LAVORI DATETIME,@DATA_COM_FINAN DATETIME, @PROROGA_6 INT
...
SELECT @DATA_FINE_LAVORI = VAL_DATA FROM DOMANDA_PAGAMENTO_REQUISITI WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO AND ID_REQUISITO = 313
...
ELSE BEGIN
  SET @RESULT=(SELECT 'DIFF'= CASE WHEN DATEDIFF(DAY, DATEADD(MM,12 +@PROROGA_3+@PROROGA_6 ,(@DATA_COM_FINAN)), @DATA_FINE_LAVORI)<=0 THEN 1 ELSE 0 END)
 END
SELECT @RESULT
END
