...
CREATE PROCEDURE [dbo].[_zImportPagamentiProgettoSigfrido95Filiere_1]
...
declare @temptable_giustificativi table (id_giust int null, cf_impresa varchar(30) null)
declare @temptable_pag_rich table (id_pag_ric int null, id_inv int null)
...
INSERT INTO [DOMANDA_DI_PAGAMENTO]
     ...
     SELECT
       ...
       gs.DATA_RILASCIO_DOMANDA_PAGAMENTO,	--,<DATA_INSERIMENTO, datetime,>
       gs.CF_OPERATORE_IMPRESA,				--,<CF_OPERATORE, varchar(16),>
       gs.DATA_RILASCIO_DOMANDA_PAGAMENTO,	--,<DATA_MODIFICA, datetime,>
       ...
       null,--1,							--,<APPROVATA, bit,>
       null,--'ND',							--,<SEGNATURA_APPROVAZIONE, varchar(100),>
       NULL,						  --,<SEGNATURA_SECONDA_APPROVAZIONE, varchar(100),>
       gs.CF_OPERATORE_ISTRUTTORIA,   --,<CF_ISTRUTTORE, varchar(16),>
       null, --GETDATE(),					  --,<DATA_APPROVAZIONE, datetime,>
...
INSERT INTO [GIUSTIFICATIVI]
           ...
           ,[ID_FILE])
           OUTPUT inserted.ID_GIUSTIFICATIVO, inserted.CF_FORNITORE into @temptable_giustificativi(id_giust, cf_impresa)
     --VALUES
		SELECT
		--ROW_NUMBER() OVER (ORDER BY VI.CODICE_FISCALE) ID_GIUST,
		gs.ID_PROGETTO_SIGEF,									--(<ID_PROGETTO, int,>
		--NULL,													--,<NUMERO, varchar(30),>
		CONVERT(VARCHAR(30), gs.ID_PROGETTO) AS NUMERO,					--,<NUMERO, varchar(30),>
		'FAT' AS TIPO,													--,<COD_TIPO, char(3),>
		GETDATE() DATA,												--,<DATA, datetime,>
		NULL NUMTRASP,													--,<NUMERO_DOCTRASPORTO, varchar(30),>
		NULL DOCTRASP,													--,<DATA_DOCTRASPORTO, datetime,>
		--gs.INV_AMMESSO,										--,<IMPONIBILE, decimal(18,2),>
		ISNULL(g.Totale_rendicontato,0) IMPONIBILE,						--,<IMPONIBILE, decimal(18,2),>
		0 AS IMP,														--,<IVA, decimal(18,2),>
		NULL ALTRIONERI,													--,<ALTRI_ONERI, decimal(18,2),>
		--'Fattura autogenerata per progetto importato id sigef ' + CONVERT(varchar(10),gs.ID_PROGETTO_SIGEF) + ' id sigfrido ' + CONVERT(varchar(10),gs.ID_PROGETTO),--,<DESCRIZIONE, varchar(255),>
		--'RIEPILOGO FATTURE PRESENTATE DAL BENEFICIARIO' + ' id sigfrido ' + CONVERT(varchar(10),gs.ID_PROGETTO),--,<DESCRIZIONE, varchar(255),>
		'riepilogo documentazione di spesa presentata dal beneficiario' AS DESCR,
		VI.CODICE_FISCALE,										--,<CF_FORNITORE, varchar(16),>
		VI.RAGIONE_SOCIALE,										--,<DESCRIZIONE_FORNITORE, varchar(255),>
		0 AS IVA,														--,<IVA_NON_RECUPERABILE, bit,>
		NULL ID_FILE													--,<ID_FILE, int,>)
		FROM _vGrad95Filiere_1 gs
		INNER JOIN 
		_GraduatoriaSigfrido95Filiere_1 g ON
		gs.ID_DOMANDA = g.ID_DOMANDA_SIGFRIDO
		INNER JOIN vIMPRESA VI ON
		g.P#IVA = VI.CODICE_FISCALE
		WHERE 
		gs.ID_DOMANDA = @id_prog_sigfrido 
     --SELECT
     --   gs.ID_PROGETTO_SIGEF,   --(<ID_PROGETTO, int,>
     --   --NULL,					--,<NUMERO, varchar(30),>
     --   CONVERT(VARCHAR(30), gs.ID_PROGETTO),					--,<NUMERO, varchar(30),>
     --   'FAT',					--,<COD_TIPO, char(3),>
     --   GETDATE(),			    --,<DATA, datetime,>
     --   NULL,				    --,<NUMERO_DOCTRASPORTO, varchar(30),>
     --   NULL,				    --,<DATA_DOCTRASPORTO, datetime,>
     --   --gs.INV_AMMESSO,			  --,<IMPONIBILE, decimal(18,2),>
     --   gs.IMPORTO_SPESA,	  --,<IMPONIBILE, decimal(18,2),>
     --   0,						--,<IVA, decimal(18,2),>
     --   NULL,					--,<ALTRI_ONERI, decimal(18,2),>
     --   --'Fattura autogenerata per progetto importato id sigef ' + CONVERT(varchar(10),gs.ID_PROGETTO_SIGEF) + ' id sigfrido ' + CONVERT(varchar(10),gs.ID_PROGETTO),--,<DESCRIZIONE, varchar(255),>
     --   --'RIEPILOGO FATTURE PRESENTATE DAL BENEFICIARIO' + ' id sigfrido ' + CONVERT(varchar(10),gs.ID_PROGETTO),--,<DESCRIZIONE, varchar(255),>
     --   'riepilogo documentazione di spesa presentata dal beneficiario',
     --   VI.CODICE_FISCALE,		--,<CF_FORNITORE, varchar(16),>
     --   VI.RAGIONE_SOCIALE,		--,<DESCRIZIONE_FORNITORE, varchar(255),>
...
tg.id_giust,
'BBP',							   --,<COD_TIPO, char(3),>
--'ND',							   --,<ESTREMI, varchar(255),>
--'Riepilogo pagamenti per progetto sigfrido id ' + CONVERT(varchar(10),gs.ID_PROGETTO),						--,<ESTREMI, varchar(255),>
'riepilogo documentazione di spesa presentata dal beneficiario',
GETDATE(),						   --,<DATA, datetime,>
--gs.INV_AMMESSO,				   --,<IMPORTO, decimal(18,2),>
--gs.INV_AMMESSO,				   --,<NETTO, decimal(18,2),>
ISNULL(g.Totale_rendicontato,0),   --,<IMPORTO, decimal(18,2),>
ISNULL(g.Totale_rendicontato,0),   --,<NETTO, decimal(18,2),>
0,--1,								--,<AMMESSO, bit,>
...
_GraduatoriaSigfrido95Filiere_1 g ON
gs.ID_DOMANDA = g.ID_DOMANDA_SIGFRIDO
INNER JOIN @temptable_giustificativi tg ON
g.P#IVA = tg.cf_impresa
WHERE
gs.ID_DOMANDA = @id_prog_sigfrido 
--SELECT
--@id_domanda_pagamento,		--(<ID_DOMANDA_PAGAMENTO, int,>
...
----gs.INV_AMMESSO,   --,<IMPORTO, decimal(18,2),>
----gs.INV_AMMESSO,   --,<NETTO, decimal(18,2),>
--gs.IMPORTO_SPESA,   --,<IMPORTO, decimal(18,2),>
--gs.IMPORTO_SPESA,   --,<NETTO, decimal(18,2),>
--0,--1,							--,<AMMESSO, bit,>
--GETDATE(),					--,<DATA_APPROVAZIONE, datetime,>
--gs.ID_OPERATORE_ISTRUTTORIA,--,<OPERATORE_APPROVAZIONE, int,>
--NULL						--,<ID_FILE, int,>)
...
INSERT INTO [PAGAMENTI_RICHIESTI]
           ...
           OUTPUT inserted.ID_PAGAMENTO_RICHIESTO, inserted.ID_INVESTIMENTO into @temptable_pag_rich(id_pag_ric, id_inv)
     --VALUES
     SELECT
		--ROW_NUMBER() OVER (ORDER BY pinv.ID_INVESTIMENTO) ID_PAG_RIC,
		@id_domanda_pagamento,		--(<ID_DOMANDA_PAGAMENTO, int,>
		...
		--gs.INV_AMMESSO,				--,<IMPORTO_RICHIESTO, decimal(18,2),>
		--gs.IMPORTO_LIQUIDATO_TOT,   --,<CONTRIBUTO_RICHIESTO, decimal(18,2),>
		ISNULL(PR.SPESA_RENDICONTATA_1_SAL_,0),			--,<IMPORTO_RICHIESTO, decimal(18,2),>
		ISNULL(PR.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],0),   --,<CONTRIBUTO_RICHIESTO, decimal(18,2),>
		...
		ISNULL(PR.SPESA_RENDICONTATA_1_SAL_,0),				--,<IMPORTO_AMMESSO, decimal(18,2),>
		ISNULL(PR.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],0),   --,<CONTRIBUTO_AMMESSO, decimal(18,2),>
		--gs.IMPORTO_LIQUIDATO_TOT,   --,<CONTRIBUTO_AMMESSO_RECUPERO_ANTICIPO, decimal(18,2),>
		0,   --,<CONTRIBUTO_AMMESSO_RECUPERO_ANTICIPO, decimal(18,2),>
		0,--1,							--,<AMMESSO, bit,>
		...
		--INNER JOIN _GraduatoriaSigfrido95Filiere_1 g ON
		--gs.ID_DOMANDA = g.ID_DOMANDA_SIGFRIDO 
		INNER JOIN 
		(
		SELECT 
		...
		VI.CODICE_FISCALE,
		VI.RAGIONE_SOCIALE,
		PINV.COSTO_INVESTIMENTO,
		PINV.CONTRIBUTO_RICHIESTO,
		gs.SPESA_RENDICONTATA_1_SAL_,
		gs.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_]
		FROM 
		PRIORITA_X_INVESTIMENTI PRI
		INNER JOIN vIMPRESA VI ON
		PRI.VAL_TESTO = VI.CODICE_FISCALE
		INNER JOIN PIANO_INVESTIMENTI PINV ON
		PRI.ID_INVESTIMENTO = PINV.ID_INVESTIMENTO
		INNER JOIN _GraduatoriaSigfrido95Filiere_1 gs ON
		gs.P#IVA = PRI.VAL_TESTO
		WHERE 
		--PINV.ID_PROGETTO = 1158
		gs.ID_DOMANDA_SIGFRIDO = @id_prog_sigfrido
		AND 
		PINV.AMMESSO IS NOT NULL
		)
		PR ON
		PR.ID_INVESTIMENTO = PINV.ID_INVESTIMENTO
		WHERE 
		pinv.AMMESSO = 1
...
----gs.INV_AMMESSO,				--,<IMPORTO_RICHIESTO, decimal(18,2),>
----gs.IMPORTO_LIQUIDATO_TOT,   --,<CONTRIBUTO_RICHIESTO, decimal(18,2),>
--gs.IMPORTO_SPESA_RICHIESTO,			--,<IMPORTO_RICHIESTO, decimal(18,2),>
--gs.CONTRIBUTO_SPESA_RICHIESTO,   --,<CONTRIBUTO_RICHIESTO, decimal(18,2),>
--NULL,						--,<CONTRIBUTO_RICHIESTO_RECUPERO_ANTICIPO, decimal(18,2),>
--NULL,						--,<IMPORTO_COMPUTO_METRICO_AMMESSO, decimal(18,2),>
----gs.INV_AMMESSO,				--,<IMPORTO_AMMESSO, decimal(18,2),>
--gs.IMPORTO_SPESA_AMMESSO,				--,<IMPORTO_AMMESSO, decimal(18,2),>
--gs.CONTRIBUTO_SPESA_CONCESSO,   --,<CONTRIBUTO_AMMESSO, decimal(18,2),>
----gs.IMPORTO_LIQUIDATO_TOT,   --,<CONTRIBUTO_AMMESSO_RECUPERO_ANTICIPO, decimal(18,2),>
--0,   --,<CONTRIBUTO_AMMESSO_RECUPERO_ANTICIPO, decimal(18,2),>
--0,--1,							--,<AMMESSO, bit,>
--gs.CF_OPERATORE_ISTRUTTORIA,--,<ISTRUTTORE, varchar(16),>
...
tg.id_giust,
--tg.cf_impresa,
--gs.DENOMINAZIONE_IMPRESA,
--gs.INV_AMMESSO,					--,<IMPORTO_RICHIESTO, decimal(18,2),>
--gs.INV_AMMESSO,			        --,<IMPORTO_AMMESSO, decimal(18,2),>
ISNULL(gs.SPESA_RENDICONTATA_1_SAL_,0),				--,<IMPORTO_RICHIESTO, decimal(18,2),>
ISNULL(gs.SPESA_RENDICONTATA_1_SAL_,0),			    --,<IMPORTO_AMMESSO, decimal(18,2),>
...
FROM 
--_vGrad95Filiere_1 gs
_GraduatoriaSigfrido95Filiere_1 gs 
INNER JOIN @temptable_giustificativi tg ON
gs.P#IVA = tg.cf_impresa
INNER JOIN 
(
SELECT 
--PRI.*,
...
PR ON
PR.CODICE_FISCALE = tg.cf_impresa
INNER JOIN @temptable_pag_rich pric ON
PR.ID_INVESTIMENTO = pric.id_inv
WHERE 
gs.ID_DOMANDA_SIGFRIDO = @id_prog_sigfrido
...
gs.CONTRIBUTO_SPESA_RICHIESTO,       --,<IMPORTO_AMMISSIBILE, decimal(18,2),>
0,								--,<IMPORTO_SANZIONE, decimal(18,2),>
0,								--,<IMPORTO_RECUPERO_ANTICIPO, decimal(18,2),>
gs.CONTRIBUTO_SPESA_CONCESSO,       --,<IMPORTO_AMMESSO, decimal(18,2),>
gs.CONTRIBUTO_SPESA_CONCESSO,       --,<IMPORTO_LIQUIDATO, decimal(18,2),>
0,--1,								--,<FLAG_LIQUIDATO, bit,>
NULL							--,<ID_DECRETO, int,>)
FROM _vGrad95Filiere_1 gs
WHERE 
gs.ID_DOMANDA = @id_prog_sigfrido
...
INSERT INTO [DOMANDA_PAGAMENTO_LIQUIDAZIONE]
     ...
     --VALUES
SELECT
--liq.DENOMINAZIONE_IMPRESA,
@id_domanda_pagamento,				--(<ID_DOMANDA_PAGAMENTO, int,>
gs.ID_PROGETTO_SIGEF,				--,<ID_PROGETTO, int,>
liq.ID_IMPRESA,						--,<ID_IMPRESA_BENEFICIARIA, int,>
ISNULL(liq.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],0),       --,<RICHIESTA_MANDATO_IMPORTO, decimal(18,2),>
...
liq.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],       --,<MANDATO_IMPORTO, decimal(18,2),>
NULL,								--,<MANDATO_ID_FILE, int,>
GETDATE(),							--,<QUIETANZA_DATA, datetime,>
liq.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],       --,<QUIETANZA_IMPORTO, decimal(18,2),>
0 --1								--,<LIQUIDATO, bit,>)
FROM _vGrad95Filiere_1 gs
INNER JOIN 
( 
...
g.DENOMINAZIONE_IMPRESA,
g.[_c)_=_(BA)_CONTRIBUTO_1_SAL_DA_LIQUIDARE_],
VI.ID_IMPRESA
FROM 
...
gs.ID_DOMANDA = liq.ID_DOMANDA_SIGFRIDO 
WHERE 
gs.ID_DOMANDA = @id_prog_sigfrido
--SELECT
--@id_domanda_pagamento,				--(<ID_DOMANDA_PAGAMENTO, int,>
...
--gs.CONTRIBUTO_SPESA_CONCESSO,       --,<RICHIESTA_MANDATO_IMPORTO, decimal(18,2),>
--'nd',								--,<RICHIESTA_MANDATO_SEGNATURA, varchar(100),>
--GETDATE(),							--,<RICHIESTA_MANDATO_DATA, datetime,>
--NULL,								--,<MANDATO_NUMERO, varchar(10),>
--GETDATE(),							--,<MANDATO_DATA, datetime,>
--gs.CONTRIBUTO_SPESA_CONCESSO,       --,<MANDATO_IMPORTO, decimal(18,2),>
--NULL,								--,<MANDATO_ID_FILE, int,>
--GETDATE(),							--,<QUIETANZA_DATA, datetime,>
--gs.CONTRIBUTO_SPESA_CONCESSO,       --,<QUIETANZA_IMPORTO, decimal(18,2),>
--0 --1								--,<LIQUIDATO, bit,>)
--FROM _vGrad95Filiere_1 gs
--WHERE 
--gs.ID_DOMANDA = @id_prog_sigfrido
...
--select @id_prog_sigef = id_progetto_sigef from _vGrad95Filiere_1 where id_domanda = @id_prog_sigfrido
