CREATE  PROCEDURE [dbo].[calcoloRequisitoPagamento112_6]
@ID_DOMANDA_PAGAMENTO int
AS
BEGIN

DECLARE @ID_PROGETTO INT, @CODICE INT,@RESULT INT,@PROROGA_12 INT,
		@DATA_FINE_INTERVENTI DATETIME,@DATA_GRADUATORIA DATETIME, @PROROGA_6 INT
...
IF((@DATA_FINE_INTERVENTI)<=(SELECT DATA_MODIFICA FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO ))
 BEGIN 
  SET @RESULT=(SELECT 'DIFF'= CASE WHEN DATEDIFF(DAY, DATEADD(MM,40,(@DATA_GRADUATORIA)),(SELECT DATA_MODIFICA FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO ))<=0 THEN 1 ELSE 0 END)
  IF(@RESULT =0 ) BEGIN 
		IF(@PROROGA_6>0) SET @RESULT=(SELECT 'DIFF'= CASE WHEN DATEDIFF(DAY, DATEADD(MM,46,(@DATA_GRADUATORIA)),(SELECT DATA_MODIFICA FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO ))<=0 THEN 1 ELSE 0 END)
	    ELSE IF(@PROROGA_12>0) SET @RESULT=(SELECT 'DIFF'= CASE WHEN DATEDIFF(DAY, DATEADD(MM,49,(@DATA_GRADUATORIA)),(SELECT DATA_MODIFICA FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO ))<=0 THEN 1 ELSE 0 END)
	 END
	END 
SELECT @RESULT
END
