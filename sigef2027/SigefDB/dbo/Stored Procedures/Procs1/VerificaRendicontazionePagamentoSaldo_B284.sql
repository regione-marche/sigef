CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B284]
(
...
DECLARE @INV_VIABILITA decimal(18,2) ,
		@INV_SELVICOLTURA decimal(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN


SET @INV_VIABILITA=(SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
						WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI INNER JOIN
								...
								CI.ID_BANDO = 284 AND DI.COD_DETTAGLIO = 'a7'))
															
SET @INV_SELVICOLTURA=(SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
						WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI INNER JOIN
								DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE
								CI.ID_BANDO = 284 AND DI.COD_DETTAGLIO IN ('a1','a2','a3','a4','a5','a6')))
		...
		IF(ISNULL(@INV_SELVICOLTURA,0) > 0)	
			BEGIN
				...
				BEGIN
					...
					SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Rispetto massimale investimento per Miglioramento viabilità forestale.'
				END
			END
		ELSE
			BEGIN
