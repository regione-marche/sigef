CREATE PROCEDURE [dbo].[rptRegistroRecuperi]
(
...
ISNULL(CAST(AT.NUMERO AS VARCHAR) + '|' + CONVERT(VARCHAR, AT.DATA, 103) + '|' + CAST(AT.REGISTRO AS VARCHAR), '') AS ATTO_RECUPERO,
...
ISNULL(CONVERT(VARCHAR, DATA_CERTIFICAZIONE_RECUPERO, 103), '') AS DATA_CERTIFICAZIONE_RECUPERO,
...
ISNULL(REC.IMPORTO_INTERESSI_LEGALI, 0) AS INTERESSI_LEGALI,
ISNULL(REC.IMPORTO_INTERESSI_MORA, 0) AS INTERESSI_MORA,
ISNULL(REC.IMPORTO_GESTIONE_PRATICA, 0) AS SPESE_GESTIONE_PRATICA,
ISNULL(REC.IMPORTO_RECUPERATO_UE, 0) AS FESR_RECUPERATO,
CAST(ISNULL(REC.IMPORTO_RECUPERATO_NAZIONALE * 0.7, 0.00) AS DECIMAL(15, 2)) AS STATO_RECUPERATO,
CAST(ISNULL(REC.IMPORTO_RECUPERATO_NAZIONALE * 0.3, 0) AS DECIMAL(15, 2)) AS REGIONE_RECUPERATO,
ISNULL(REC.IMPORTO_RECUPERATO_PUBBLICO, 0) AS TOTALE_PUBBLICO_RECUPERATO,
ISNULL(REC.IMPORTO_RECUPERATO_PRIVATO, 0) AS PRIVATO_RECUPERATO,
ISNULL(REC.IMPORTO_RECUPERATO_TOTALE, 0) AS TOTALE_SPESA_RECUPERATA,
ISNULL(CONVERT(VARCHAR, DATA_CONCLUSIONE, 103), '') AS DATA_FINE_RECUPERO,
CASE STABILITA_OPERAZIONI
	WHEN 1 THEN 'Sì' 
	ELSE 'No'
END AS ART71,
CASE
	WHEN STORICO = 1 THEN ISNULL(REC.IMPORTO_DA_RECUPERARE_TOTALE - IMPORTO_RECUPERATO_TOTALE, 0.00)
	ELSE 0.00
END AS SPESA_PASSATA_RITIRO
FROM VREGISTRO_RECUPERO REC
INNER JOIN VIMPRESA IMP ON REC.ID_IMPRESA = IMP.ID_IMPRESA
LEFT JOIN ATTI AT ON REC.ID_ATTO_RECUPERO = AT.ID_ATTO
...
-- EXEC [rptRegistroRecuperi] 45, null, null, null;