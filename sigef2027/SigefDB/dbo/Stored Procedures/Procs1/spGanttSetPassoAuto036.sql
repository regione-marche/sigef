...
-- Description:	Aggiorna i passi GANTT che prendono i valori in automatico dal SIGEF (ID_TIPO = 36: pubblicazione bando)
...
-- =============================================
CREATE PROCEDURE [dbo].[spGanttSetPassoAuto036]
	@idStep int 
AS
BEGIN
	SET NOCOUNT ON;
	...
	WHERE GANTT_STEPS.ID_STEP=@idStep and GANTT_STEPS.ID_TIPO_PASSO in (36)
	if (@dataLastEditStepsGANTT is null) set @dataLastEditStepsGANTT = '19000101'

	--  Legge data di pubblicazione SIGEF sulla base del GANTT e bando SIGEF collegati allo step @idStep
	SELECT  @dataPubblicaSIGEF = BS.DATA -- ,BS.ID_BANDO, GANTT.ID_GANTT
	...
	WHERE        (BS.COD_STATO = 'L') and GANTT_STEPS.ID_STEP=@idStep
