CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B278]
(
...
--*Attività di rilevazione ed acquisizione dei dati e delle informazioni territoriali max 10% del costo totale di progetto
--*Spese relative al personale interno max 10% del costo totale di progetto

--*Intrevento b) Azioni Promozionali - Spese per servizi e consulenze per la predisposizione del piano promozionale e 
--   di comunicazione e per la pianificazione operativa delle attività, max 20% del costo complessivo del progetto
...
-- Attività di rilevazione ed acquisizione dei dati e delle informazioni territoriali max 10% del costo totale di progetto
DECLARE @ACQUI_DATI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @ACQUI_DATI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT DI.ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											WHERE CI.ID_BANDO = 278 AND DI.COD_DETTAGLIO = '1' AND CI.CODICE = 'a1')
					
	IF isnull(@ACQUI_DATI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Attività di rilevazione ed acquisizione dei dati e delle informazioni territoriali max 10% del costo totale di progetto.'
	END
END
---------------------------------------------------------------------------------------------------
-- Spese relative al personale interno max 10% del costo totale di progetto.
DECLARE @COSTI_PERS_INTERNO int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_PERS_INTERNO=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											INNER JOIN SPECIFICA_INVESTIMENTI SPI ON (SPI.ID_DETTAGLIO_INVESTIMENTO = DI.ID_DETTAGLIO_INVESTIMENTO)
											WHERE CI.ID_BANDO = 278 AND SPI.COD_SPECIFICA in ('11','21','31','41','61','71'))
					
	IF isnull(@COSTI_PERS_INTERNO,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese relative al personale interno max 10% del costo totale di progetto.'
	END
END
---------------------------------------------------------------------------------------------------
-- Intrevento b) Azioni Promozionali - Spese per servizi e consulenze per la predisposizione del piano promozionale e di comunicazione e per la pianificazione operativa delle attività, max 20% del costo complessivo del progetto
DECLARE @COSTI_CONSULENZE int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_CONSULENZE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT DI.ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											WHERE CI.ID_BANDO = 278 AND DI.COD_DETTAGLIO = '1' AND CI.CODICE = 'b1')
					
	IF isnull(@COSTI_CONSULENZE,0)>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Intrevento b) Azioni Promozionali - Spese per servizi e consulenze per la predisposizione del piano promozionale e di comunicazione e per la pianificazione operativa delle attività, max 20% del costo complessivo del progetto.'
	END
END
