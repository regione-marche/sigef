CREATE PROCEDURE [dbo].[calcoloStepVariante112_121_5]
@ID_VARIANTE int
AS
BEGIN
...
SELECT ( (((isnull(@PunteggioPrioritaA,0)/100) * @PesoPrioritaA) / @ValoreMAXPrioritaA)  
        + (((isnull(@PunteggioPrioritaB,0)/100) * @PesoPrioritaB) / @ValoreMAXPrioritaB)  
        + (((isnull(@PunteggioPrioritaC,0)/100) * @PesoPrioritaC) / @ValoreMAXPrioritaC)  
       ) AS PUNTEGGIO
