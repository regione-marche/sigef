CREATE PROCEDURE [dbo].[calcoloStep112_5]
(
@IdProgetto int,
...
SET @CONTRIBUTO_RICHIESTO = 
   (SELECT SUM(ISNULL (PIANO_INVESTIMENTI.CONTRIBUTO_RICHIESTO ,0)) as CONTRIBUTO
	FROM  PIANO_INVESTIMENTI    
		INNER JOIN (SELECT DISTINCT PROGETTO.ID_PROGETTO, FLAG_DEFINITIVO 
					FROM PROGETTO 
						INNER JOIN  PROGETTO_STORICO  ON PROGETTO.ID_PROGETTO = PROGETTO_STORICO.ID_PROGETTO 
					WHERE (PROGETTO_STORICO.ID_PROGETTO not in ( select Id_Progetto from vPROGETTO PS where  PS.COD_STATO  IN ('P','Q','B', 'N','R'))
							OR (PROGETTO.ID_PROGETTO = @IdProgetto  )) AND ID_IMPRESA  IN (SELECT ID_IMPRESA FROM PROGETTO WHERE ID_PROGETTO = @IdProgetto ) 
							AND FLAG_PREADESIONE =0 AND PROGETTO.ID_BANDO NOT IN (69, 40, 41, 42,88, 89))AS PROG ON PROG.ID_PROGETTO = PIANO_INVESTIMENTI.ID_PROGETTO 
