CREATE  PROCEDURE [dbo].[calcoloStep111_121_3]
...
--------------------------------------------------------------------------------------------------------------
 ...
 DECLARE CORSO CURSOR FOR
  ( 
		...
		FROM vPIANO_INVESTIMENTI PI WHERE ID_PROGETTO =@IdProgetto AND ((ID_INVESTIMENTO_ORIGINALE IS NULL and @fase_istruttoria =0 and ID_VARIANTE IS NULL) OR (ID_INVESTIMENTO_ORIGINALE IS NOT NULL and @fase_istruttoria =1 and ID_VARIANTE IS NULL))
		GROUP BY  CODICE
   ) 
