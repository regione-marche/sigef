CREATE  PROCEDURE [dbo].[calcoloStepATS_2]
@IdProgetto int
AS
BEGIN
...
SET @TOTALE_INVESTIMENTO = ( SELECT     SUM(PIANO_INVESTIMENTI.COSTO_INVESTIMENTO) AS COSTO_INVESTIMENTO
							FROM  PROGETTO INNER JOIN 
								PIANO_INVESTIMENTI ON PROGETTO.ID_PROGETTO = PIANO_INVESTIMENTI.ID_PROGETTO 
							WHERE  PROGETTO.ID_PROGETTO= @IdProgetto AND ID_DETTAGLIO_INVESTIMENTO NOT IN (722,727,732,737,742) 
...
SET @TOTALE_9 = ( SELECT ISNULL(SUM(PI.COSTO_INVESTIMENTO),0)
				...
				AND ((PI.ID_INVESTIMENTO_ORIGINALE IS NULL AND P.FLAG_DEFINITIVO = 0 ) 
						OR (PI.ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND P.FLAG_DEFINITIVO = 1 AND ID_VARIANTE IS NULL) )
				AND PI.ID_SPECIFICA_INVESTIMENTO IN (348,409,418,427,436 ))
