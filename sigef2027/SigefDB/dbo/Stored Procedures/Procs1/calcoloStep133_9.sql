CREATE PROCEDURE [dbo].[calcoloStep133_9]
(
	@IdProgetto int,
...
AS
BEGIN
-- TOTALE PROGETTO PER ANNUALITà CORRISPONDENTE CON IL PROGETTO PLURIENNALE
DECLARE @RESULT INT, @ID_PROGETTO_PLURI INT,
		@ID_ANNUALITA INT, @ANNO_MIN INT,@ANNO_MAX INT,
...
-- CONTROLLO L'ANNUALITA -->ID PRIORITA 768
SELECT @ID_ANNUALITA = PXI.ID_VALORE FROM PIANO_INVESTIMENTI INV 
...
--TROVO L'ANNUALITà PIù PICCOLA e LA PIù GRANDE DEFINITA NEL PROGETTO PLURIENNALE (PRENDO ID_VALORE)
SELECT @ANNO_MIN = PXI.ID_VALORE FROM PIANO_INVESTIMENTI INV 
		INNER JOIN PRIORITA_X_INVESTIMENTI PXI ON PXI.ID_INVESTIMENTO= INV.ID_INVESTIMENTO 
		INNER JOIN VALORI_PRIORITA VP ON VP.ID_VALORE = PXI.ID_VALORE AND VP.ID_PRIORITA =404 
...
WHERE ID_PROGETTO = @IdProgetto AND COD_TIPO_INVESTIMENTO =1 AND ((@FASE_ISTRUTTORIA=0 AND ID_INVESTIMENTO_ORIGINALE IS NULL AND ID_VARIANTE IS NULL) OR(@FASE_ISTRUTTORIA=1 AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL))

IF @ID_ANNUALITA= 808 BEGIN--INVESTIMENTI CON L'ANNO PIU' PICCOLO
	SELECT @COSTO_ANNO_PLURIENNALE= SUM(COSTO_INVESTIMENTO) FROM PIANO_INVESTIMENTI INV 
			INNER JOIN PRIORITA_X_INVESTIMENTI PXI ON PXI.ID_INVESTIMENTO = INV.ID_INVESTIMENTO AND PXI.ID_PRIORITA=404 AND PXI.ID_VALORE=@ANNO_MIN
...
--SELECT @COSTO_ANNO_PLURIENNALE,@COSTO_ANNUALITA
END 
ELSE IF @ID_ANNUALITA =809 BEGIN -- 809 = secondo anno 
	 SELECT @COSTO_ANNO_PLURIENNALE= SUM(COSTO_INVESTIMENTO) FROM PIANO_INVESTIMENTI INV 
			INNER JOIN PRIORITA_X_INVESTIMENTI PXI ON PXI.ID_INVESTIMENTO = INV.ID_INVESTIMENTO AND PXI.ID_PRIORITA=404 AND PXI.ID_VALORE!= @ANNO_MAX AND PXI.ID_VALORE!= @ANNO_MIN
	 WHERE ID_PROGETTO = @ID_PROGETTO_PLURI AND COD_TIPO_INVESTIMENTO =1 AND ((@FASE_ISTRUTTORIA=0 AND ID_INVESTIMENTO_ORIGINALE IS NULL AND ID_VARIANTE IS NULL) 
			OR(@FASE_ISTRUTTORIA=1 AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL))END 
ELSE BEGIN -- TERZO ANNO:CONSIDERO L'ANNUALITà MAGGIORE DEFINITA NEL PROGETTO PLURIENNALE
		SELECT @COSTO_ANNO_PLURIENNALE= SUM(COSTO_INVESTIMENTO) FROM PIANO_INVESTIMENTI INV 
				INNER JOIN PRIORITA_X_INVESTIMENTI PXI ON PXI.ID_INVESTIMENTO = INV.ID_INVESTIMENTO AND PXI.ID_PRIORITA=404 AND PXI.ID_VALORE= @ANNO_MAX
		WHERE ID_PROGETTO = @ID_PROGETTO_PLURI AND COD_TIPO_INVESTIMENTO =1 AND ((@FASE_ISTRUTTORIA=0 AND ID_INVESTIMENTO_ORIGINALE IS NULL AND ID_VARIANTE IS NULL) 
				OR(@FASE_ISTRUTTORIA=1 AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL))END
