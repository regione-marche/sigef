CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B273]
(
...
-- Costo investimento ammissibile >= €30.000 e <= €150.000 al netto dell`IVA.
...
--Costo investimento ammissibile >= €30.000 e <= €150.000 al netto dell`IVA.
IF ((isnull(@COSTO_INVESTIMENTI,0) < 30000) or (isnull(@COSTO_INVESTIMENTI,0) > 150000))BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo investimento ammissibile all’aiuto < € 30.000,00 o > 150.000,00 Euro.'
END
---------------------------------------------------------------------------------------------------
-- Spese generali massimo 10% del costo totale del progetto
...
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_GENERALI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		...
		DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   	    WHERE CI.ID_BANDO = 273 AND DI.COD_DETTAGLIO in ('e'))
					
	IF isnull(@COSTI_GENERALI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
