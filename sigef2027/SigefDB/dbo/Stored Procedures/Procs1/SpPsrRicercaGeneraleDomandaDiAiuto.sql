...
CREATE PROCEDURE [dbo].[SpPsrRicercaGeneraleDomandaDiAiuto]
(
...
AS
	...
	FROM (
		...
		INNER JOIN STATO_PROGETTO SP ON S.COD_STATO=SP.COD_STATO									
		...
		INNER JOIN zPSR_ALBERO Z ON PR.COD_TIPO=Z.CODICE
		...
		--Ricerca per tipo domanda di pagamento
		LEFT JOIN  (
			SELECT ID_PROGETTO, COD_TIPO
			FROM DOMANDA_DI_PAGAMENTO  WHERE SEGNATURA IS NOT NULL GROUP BY ID_PROGETTO,COD_TIPO
		) DPTIPO on P.ID_PROGETTO=DPTIPO.ID_PROGETTO
		...
		WHERE ISNULL(ID_PROG_INTEGRATO,P.ID_PROGETTO)=P.ID_PROGETTO AND (@COD_STATO IS NULL OR S.COD_STATO=@COD_STATO)
			...
			AND (@COD_TIPO_DP IS NULL OR DPTIPO.COD_TIPO = @COD_TIPO_DP)
