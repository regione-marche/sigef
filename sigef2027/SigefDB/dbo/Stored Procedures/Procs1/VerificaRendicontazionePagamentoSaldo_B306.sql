CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B306]
(
...
-- Costo totale investimento >= 25.000€
-- Tipologia di intervento, selezionata nel piano degli investimenti, coerente con il bando di riferimento.
----
---------------------------------------------------------------------------------------------------
...
-- Costo totale investimento >= 25.000€
IF (isnull(@COSTO_INVESTIMENTI,0) < 25000)BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale investimento >= 25.000€'
END

---------------------------------------------------------------------------------------------------
-- Tipologia di intervento, selezionata nel piano degli investimenti, coerente con il bando di riferimento.
DECLARE @CTI DECIMAL(18,2)
SELECT @CTI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND ID_PROGRAMMAZIONE = 125
