CREATE PROCEDURE [dbo].[calcoloPrioritaVariante112_121_7]
@ID_VARIANTE INT ,
@IdProgetto int 
 AS
BEGIN
-- CALCOLO Punteggio 112 
...
(
 SELECT    ID_SETTORE_PRODUTTIVO, TIPO_AREA,  sum(Costo)
	FROM  (SELECT     SUM(PI.COSTO_INVESTIMENTO) AS Costo, ISNULL(vComuni.TIPO_AREA, @TIPO_AREA_PREVALENTE)  AS TIPO_AREA ,  
			 'ID_SETTORE_PRODUTTIVO' = CASE when PI.ID_SETTORE_PRODUTTIVO = 216 THEN 20 else PI.ID_SETTORE_PRODUTTIVO
						END
		FROM CATASTO_TERRENI INNER JOIN
