...
CREATE PROCEDURE [dbo].[SpCalcoloContributoRichiestoDisponibilePagamento]
(
	@ID_PROGETTO INT, --ID_PROGETTO CORRELATO
	@ID_DOMANDA_PAGAMENTO INT,
	@COD_TIPO_INVESTIMENTO INT,
	@ID_PAGAMENTO_RICHIESTO INT =NULL
)
AS
DECLARE @CONTRIBUTO_RICHIESTO_DISPONIBILE DECIMAL(18,2)/*
...
SET @CONTRIBUTO_RICHIESTO_DISPONIBILE =dbo.calcoloContributoTotaleProgettoCorrelato(@ID_PROGETTO,1,@ID_VARIANTE)
...
IF(@CONTRIBUTO_AMMESSO_IN_GRADUATORIA IS NOT NULL AND @CONTRIBUTO_AMMESSO_IN_GRADUATORIA<@CONTRIBUTO_RICHIESTO_DISPONIBILE) 
	SET @CONTRIBUTO_RICHIESTO_DISPONIBILE=@CONTRIBUTO_AMMESSO_IN_GRADUATORIA
	
--SOTTRAGGO IL CONTRIBUTO AMMESSO DELLE DOMANDE PRECEDENTI.. 
DECLARE @CONTRIBUTO_AMMESSO DECIMAL(18,2),@CONTRIBUTO_RICHIESTO_ALTRI_INVESTIMENTI DECIMAL(18,2)
...
AND DP.COD_TIPO!='ANT'
--E IL RICHIESTO DI QUELLA ATTUALE SUGLI ALTRI INVESTIMENTI
SELECT @CONTRIBUTO_RICHIESTO_ALTRI_INVESTIMENTI=SUM(PR.CONTRIBUTO_RICHIESTO) FROM PAGAMENTI_RICHIESTI  PR
...
SET @CONTRIBUTO_RICHIESTO_DISPONIBILE=@CONTRIBUTO_RICHIESTO_DISPONIBILE-ISNULL(@CONTRIBUTO_AMMESSO,0)
-ISNULL(@CONTRIBUTO_RICHIESTO_ALTRI_INVESTIMENTI,0)
IF(@CONTRIBUTO_RICHIESTO_DISPONIBILE<0) SET @CONTRIBUTO_RICHIESTO_DISPONIBILE=0
SELECT @CONTRIBUTO_RICHIESTO_DISPONIBILE AS CONTRIBUTO_RICHIESTO_DISPONIBILE
