CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B303]
(
...
-- Costo totale investimento >= 10.000 euro e <= 150.000 euro iva esclusa
IF (isnull(@COSTO_INVESTIMENTI,0)< 10000) or (isnull(@COSTO_INVESTIMENTI,0) > 150000) BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale investimento >= 10.000 euro e <= 150.000 euro iva esclusa'
END
---------------------------------------------------------------------------------------------------
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_TERRENI=SUM(IMPORTO_TOTALE_RENDICONTATO) 
	...
	INNER JOIN CODIFICA_INVESTIMENTO CI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)   
	WHERE DI.COD_DETTAGLIO = '2' AND CI.CODICE = '2b'
	
	IF @COSTO_TERRENI>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
