CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B309]
(
...
-- Investimenti per avviamento servizi <= 20% totale investimenti
DECLARE @COSTI_AVVIAMENTO int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_AVVIAMENTO=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 309 AND CODICE = '3')
					
	IF isnull(@COSTI_AVVIAMENTO,0)>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - -- Investimenti per avviamento servizi <= 20% totale investimenti'
	END
END
---------------------------------------------------------------------------------------------------

-- Il piano degli investimenti rispetta il limite minimo di contributo ammesso di € 10.000,00 e il limite max di € 50.000,00 al netto dell’ IVA come previsto al paragrafo 8.2 del bando
DECLARE @COSTO_CONTRIBUTO_TOTALE_RENDICONTATO DECIMAL(18,2)
IF ISNULL(@COSTO_INVESTIMENTI,0)>0 BEGIN

	SELECT @COSTO_CONTRIBUTO_TOTALE_RENDICONTATO=SUM(CONTRIBUTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1

	IF ((isnull(@COSTO_CONTRIBUTO_TOTALE_RENDICONTATO,0)< 10000) OR (isnull(@COSTO_CONTRIBUTO_TOTALE_RENDICONTATO,0)> 50000)) BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il piano degli investimenti rispetta il limite minimo di contributo ammesso di € 10.000,00 e il limite max di € 50.000,00 al netto dell’ IVA'
	END
END
