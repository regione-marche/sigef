-- SALDO
--=== Data di ultimazione degli interventi programmati (entro 15 mesi dal ricevimento dell`atto di concessione) ===--
...
CREATE PROCEDURE [dbo].[calcoloRequisitoPagamento_B201_2]
...
SELECT @DATA_FINE_LAVORI = VAL_DATA FROM DOMANDA_PAGAMENTO_REQUISITI WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO AND ID_REQUISITO = 285
...
ELSE BEGIN
  SET @RESULT=(SELECT 'DIFF'= CASE WHEN DATEDIFF(DAY, DATEADD(MM,15 +@PROROGA_3+@PROROGA_6 ,(@DATA_COM_FINAN)), @DATA_FINE_LAVORI)<=0 THEN 1 ELSE 0 END)
 END
SELECT @RESULT
END
