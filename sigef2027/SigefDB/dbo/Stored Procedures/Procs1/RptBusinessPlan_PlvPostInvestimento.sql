CREATE PROCEDURE [dbo].[RptBusinessPlan_PlvPostInvestimento] 
@ID_Impresa int,
@ID_Domanda int,
@TipoPLV int
AS
...
DECLARE @CountPac int
	...
	SET @Count = (SELECT COUNT(ID_PLV) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1)     						
	 ...
	 BEGIN
		...
		IF (@TipoPLV = 0) -- Plv Piano Colturale					 			   
			   ...
			   SELECT ANNO, 
					  ...
					  dbo.MqAEttari((SELECT SUM(SAU) 
									...
									WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1 AND COD_PRODOTTO IS NOT NULL
										AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1))) AS TOT_SAU
						, CUAA_TRASFORMATORE,
						0 AS NUM_CAPI_FILIERA
			   FROM vPLV_AZIENDALE PLVA LEFT JOIN UNITA_DI_MISURA UM ON (PLVA.ID_UNITA_MISURA = UM.ID_UNITA_MISURA)

			   WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1 AND COD_PRODOTTO IS NOT NULL
					 AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1)
		...
		ELSE IF (@TipoPLV = 1) -- Plv Zootecnica		
			 ...
			 SELECT ANNO, 
			   ...
			   WHERE ID_PROGETTO = @ID_Domanda AND ID_CLASSE_ALLEVAMENTO IS NOT NULL AND PREVISIONALE = 1
					 AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1)
		...
		ELSE IF (@TipoPLV = 2)  -- Plv Attività connesse	
			   ...
			   WHERE ID_PROGETTO = @ID_Domanda AND ID_ATTIVITA_CONNESSA IS NOT NULL	AND PREVISIONALE = 1
                     AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1)
		...
		ELSE IF (@TipoPLV = 3)  -- Contributi P.A.C.	
		   ...
		   IF (@ID_Domanda IS NOT NULL) -- a livello di domanda
			begin
			 SET @CountPac = (SELECT COUNT(ID_PLV) FROM vPLV_AZIENDALE   WHERE ID_PROGETTO = @ID_Domanda AND CODICE_PAC IS NOT NULL AND PREVISIONALE = 1 AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1))     						
			...
			IF (@CountPac > 0)
			   ...
			   WHERE ID_PROGETTO = @ID_Domanda AND CODICE_PAC IS NOT NULL AND PREVISIONALE = 1
                     AND ANNO = (SELECT MAX(ANNO) FROM vPLV_AZIENDALE WHERE ID_PROGETTO = @ID_Domanda AND PREVISIONALE = 1)
