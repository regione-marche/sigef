--SAL  111 a assam   
-- acquisto di servizi sul mercato per una quota non superiore al 10% del costo del progetto.

CREATE   PROCEDURE [dbo].[calcoloRequisitoPagamento111_6]
...
SET @IMPORTO_RICHIESTO=(SELECT SUM(ISNULL(IMPORTO_RICHIESTO,0) )
						 ...
						 WHERE ID_PROGETTO = @ID_PROGETTO AND ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO AND
							  ...
							  ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)   
															WHERE CI.ID_BANDO = (SELECT ID_BANDO FROM PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO) AND DI.COD_DETTAGLIO = '4'))
