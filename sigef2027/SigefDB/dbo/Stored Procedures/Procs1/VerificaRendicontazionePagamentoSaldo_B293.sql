CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B293]
(
...
----===  Costo totale investimenti per acquisto terreni e fabbricati <= 10 totale progetto ==---
DECLARE @COSTO_ACQUISTO_TERR int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_ACQUISTO_TERR=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 															
											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   						WHERE CI.ID_BANDO = 293 AND DI.COD_DETTAGLIO = '1g' and CI.CODICE = '1')
					
	IF isnull(@COSTO_ACQUISTO_TERR,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale investimenti per acquisto terreni e fabbricati <= 10 totale progetto.'
	END
END
