CREATE PROCEDURE [dbo].[ZQuadriXDomandaFindFind]
(
...
AS
	...
	FROM vQUADRIDOMANDA
	WHERE 
		((@ID_QUADROEQUALTHIS IS NULL) OR ID_QUADRO = @ID_QUADROEQUALTHIS) AND 
		((@ID_DOMANDAEQUALTHIS IS NULL) OR ID_DOMANDA = @ID_DOMANDAEQUALTHIS) AND 
		((@ORDINEEQUALTHIS IS NULL) OR ORDINE = @ORDINEEQUALTHIS)

', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'ZQuadriXDomandaFindFind';
