CREATE PROCEDURE [dbo].[zCUPGetDescrizioneLavoriPubblici]
(
	@IdProgetto INT 
)
AS
...
IP.NR_IMPRESE AS nr_imprese,
CASE WHEN IP.NR_IMPRESE = 1 THEN 'S' ELSE 'N' END AS str_infrastr_unica
FROM 
vPROGETTO P
...
(
	SELECT 
	P.ID_PROGETTO,
	ISNULL(NR_IMP.NR_IMPRESE, 1) AS NR_IMPRESE 
	FROM 
	PROGETTO P
	LEFT OUTER JOIN 
	PRIORITA_X_PROGETTO PP ON
	P.ID_PROGETTO = PP.ID_PROGETTO
	AND
	PP.ID_PRIORITA = 333
	LEFT OUTER JOIN 
	AGGREGAZIONE A ON
	...
	(
		SELECT 
		COUNT(ID_IMPRESA) AS NR_IMPRESE,
		ID_AGGREGAZIONE
		FROM
