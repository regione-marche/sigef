CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B324]
(
...
----------------------------------------------------------------------------------------------------------------
-- --Il piano degli investimenti rispetta il limite massimo di € 100.000,00 al netto dell’ IVA come previsto al paragrafo 8 del bando
IF (isnull(@COSTO_INVESTIMENTI,0) > 100000) BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il piano degli investimenti rispetta il limite massimo di € 100.000,00 al netto dell’ IVA'
END
----------------------------------------------------------------------------------------------------------------
...
SELECT @INTERVENTO_A=ISNULL(COUNT(DISTINCT(ID_CODIFICA_INVESTIMENTO)),0) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN 
		(SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 324 AND CODICE IN ('a','b','c'))

SELECT @INTERVENTO_B=ISNULL(COUNT(DISTINCT(ID_CODIFICA_INVESTIMENTO)),0) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN 
		(SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 324 AND CODICE IN ('d','e','f'))

IF((ISNULL(@INTERVENTO_A,0) >= 1) AND (ISNULL(@INTERVENTO_B,0) >= 1)) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il beneficiario ha inserito investimenti per un solo intervento'
END
