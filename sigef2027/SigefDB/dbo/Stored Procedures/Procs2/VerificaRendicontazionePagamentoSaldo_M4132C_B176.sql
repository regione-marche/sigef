CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M4132C_B176]
(
...
---------------------------------------------------------------------------------------------------
-- La spesa ammissibile deve essere inferiore ad euro 150.000,00
...
IF @COSTO_INVESTIMENTI>150000 BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - L`ammontare rendicontato sulla misura 4132c deve essere inferiore a 150.000 €.'
END
	
------------------------------------------------------------------------------------------------

-- 2.  Spese relative a guide su supporto cartaceo e multimediale <= 50.000€

DECLARE @COSTO_INVESTIMENTI_GUIDE DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_GUIDE=SUM(IMPORTO_TOTALE_RENDICONTATO) 
	FROM @INVESTIMENTI_TEMP INV	INNER JOIN CODIFICA_INVESTIMENTO CI ON (INV.ID_CODIFICA_INVESTIMENTO = CI.ID_CODIFICA_INVESTIMENTO)   
	WHERE CI.CODICE IN( '01')
	
	IF @COSTO_INVESTIMENTI_GUIDE>50000 BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo relativo alle guide su supporto cartaceo e multimediale deve essere inferiore a 50.000€.'
	END
END
