CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B329]
(
...
-- Costo totale investimenti <= 150.000€
IF (isnull(@COSTO_INVESTIMENTI,0) > 150000)BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale investimenti <= 150.000€'
END
---------------------------------------------------------------------------------------------------
--Costo acquisto terreni <= 10% totale progetto
DECLARE @COSTO_TERRENI DECIMAL(18,2), @COSTO_STRUTTURALI DECIMAL(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_TERRENI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 
		                                                                     ...
		                                                                     WHERE ID_BANDO = 329 AND DI.COD_DETTAGLIO IN ('b8','8b'))
		                                                                     
		IF (((isnull(@COSTO_TERRENI,0) * 100) / @COSTO_INVESTIMENTI) > 10 ) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo acquisto terreni <= 10% totale progetto.'
	END
END
