CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B447]
(
...
--Spese generali massimo 10% del costo totale del progetto
--Costo investimento  <= €150.000 al netto dell`IVA.
...
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	...
	IF (isnull(@COSTO_INVESTIMENTI,0)> 150000) BEGIN
		...
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo investimento <= €150.000 al netto dell`IVA..'
	END
END
----------------------------------------------------------------------------------------------------------------

----Spese generali massimo 10% del costo totale del progetto
DECLARE @COSTO_SPESE_GENERALI DECIMAL(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_SPESE_GENERALI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 
		                                                                     ...
		                                                                     WHERE ID_BANDO = 447 AND DI.COD_DETTAGLIO = 'e')

			IF isnull(@COSTO_SPESE_GENERALI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese generali massimo 10% del costo totale del progetto).'
	END
END
