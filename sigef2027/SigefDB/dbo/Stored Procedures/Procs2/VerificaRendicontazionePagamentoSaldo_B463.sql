CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B463]
(
...
IF isnull(@COSTO_INVESTIMENTI,0)>150000 BEGIN
	...
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo dell`investimento ammissibile supera 150.000,00 € al netto dell`IVA.'
END
---------------------------------------------------------------------------------------------------


-- Spese per acquisto terreni nel limite del 10% del costo totale della domanda di aiuto	1	
DECLARE @COSTO_ACQUISTO_TERRENI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_ACQUISTO_TERRENI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 															
											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   						WHERE CI.ID_BANDO = 463 AND DI.COD_DETTAGLIO = 'a')
					
	IF isnull(@COSTO_ACQUISTO_TERRENI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
...
DECLARE @COSTO_AMBIENTALE int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_AMBIENTALE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 															
											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   						WHERE CI.ID_BANDO = 463 AND DI.COD_DETTAGLIO = 'd')
...
DECLARE @COSTO_ARREDI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_ARREDI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 															
											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   						WHERE CI.ID_BANDO = 463 AND DI.COD_DETTAGLIO = 'e')
