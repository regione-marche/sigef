CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M111_B120]
(
...
-- 2.  Spese per servizi esterni <= 10% del totale investimenti aventi la stessa codifica (dettaglio investimento = 5)
...
----------------------------------------------------------------------------------
-- 3. Le spese di coordinamento <= 5% totale domanda (Codifica investimento = 12)
...
END
-----------------------------------------------------------------------------------------
  -- 4.  Il costo delle spese generali non supera il 5% del costo del progetto (Codifica investimento = 13)
...
-----------------------------------------------------------------------------------------
  -- 5 - Totale spese ricognizione <= 20% totale domanda (codifica investimento = 11)
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	...
	IF @COSTO_INVESTIMENTI_RICOG>(@COSTO_INVESTIMENTI*0.2) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo delle spese ricognizione supera il 5% del costo del progetto.'
	END
END

-----------------------------------------------------------------------------------------
  -- 6 - Sono previste almeno un numero minimo di azioni informative => di 3

DECLARE @COUNT_AZIONI DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COUNT_AZIONI=COUNT(DISTINCT ID_CODIFICA_INVESTIMENTO) FROM @INVESTIMENTI_TEMP INV
	WHERE ID_CODIFICA_INVESTIMENTO NOT IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE CODICE = '11')

	IF @COUNT_AZIONI<3 BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il numero delle azioni informative è minore di 3.'
	END
END
