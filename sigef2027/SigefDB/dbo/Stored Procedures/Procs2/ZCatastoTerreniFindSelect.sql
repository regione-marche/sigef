CREATE PROCEDURE [dbo].[ZCatastoTerreniFindSelect]
(
	...
	@SuperficieCatastaleequalthis INT, 
	@Svantaggioequalthis VARCHAR(255)
)
AS
	...
	IF (@SuperficieCatastaleequalthis IS NOT NULL) BEGIN SET @sql = @sql + ' AND (SUPERFICIE_CATASTALE = @SuperficieCatastaleequalthis)'; set @lensql=@lensql+len(@SuperficieCatastaleequalthis);end;
	IF (@Svantaggioequalthis IS NOT NULL) BEGIN SET @sql = @sql + ' AND (SVANTAGGIO = @Svantaggioequalthis)'; set @lensql=@lensql+len(@Svantaggioequalthis);end;
	set @lensql=@lensql+len(@sql);
	if (@lensql<3000)
	exec sp_executesql @sql,N'@IdCatastoequalthis INT, @IdComuneequalthis INT, @FoglioCatastaleequalthis VARCHAR(255), @Particellaequalthis VARCHAR(255), @Sezioneequalthis VARCHAR(255), @Subalternoequalthis VARCHAR(255), @SuperficieCatastaleequalthis INT, @Svantaggioequalthis VARCHAR(255)',@IdCatastoequalthis , @IdComuneequalthis , @FoglioCatastaleequalthis , @Particellaequalthis , @Sezioneequalthis , @Subalternoequalthis , @SuperficieCatastaleequalthis , @Svantaggioequalthis ;
	else
		...
		WHERE 
			...
			((@Subalternoequalthis IS NULL) OR SUBALTERNO = @Subalternoequalthis) AND 
			((@SuperficieCatastaleequalthis IS NULL) OR SUPERFICIE_CATASTALE = @SuperficieCatastaleequalthis) AND 
			((@Svantaggioequalthis IS NULL) OR SVANTAGGIO = @Svantaggioequalthis)

GO
EXECUTE sp_addextendedproperty @name = N'backup', @value = NULL, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'ZCatastoTerreniFindSelect';
