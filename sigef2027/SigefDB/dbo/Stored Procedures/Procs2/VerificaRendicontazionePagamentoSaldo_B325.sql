CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B325]
(
...
-- Costo totale progetto <= 150.000€,
IF (isnull(@COSTO_INVESTIMENTI,0) > 150000)BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale progetto <= 150.000€.'
END
---------------------------------------------------------------------------------------------------
...
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	...
	SELECT @INTERVENTO_A=ISNULL(COUNT(DISTINCT(ID_CODIFICA_INVESTIMENTO)),0) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 325 AND CODICE IN ('A1','A2'))
	
	SELECT @INTERVENTO_B=ISNULL(COUNT(DISTINCT(ID_CODIFICA_INVESTIMENTO)),0) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 325 AND CODICE IN ('B1','B2'))
					
	IF((@INTERVENTO_A >= 1) AND (@INTERVENTO_B >= 1)) BEGIN
		SET @TIPO_RISULTATO='N'
