CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B354]
(
...
--Spese tecniche <= 12% totale investimenti
DECLARE @SPESE_TEC DECIMAL(18,2)
DECLARE @ALTRI_INV DECIMAL(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @SPESE_TEC=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 354 AND CODICE = 'd')
	
	SELECT @ALTRI_INV=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 354 AND CODICE IN ('a','b','c'))
					
	IF isnull(@SPESE_TEC,0)>(@ALTRI_INV*0.12) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese tecniche <= 12% totale investimenti.'
	END
END
