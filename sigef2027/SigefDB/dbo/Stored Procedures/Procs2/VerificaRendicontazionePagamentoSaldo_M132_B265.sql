CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M132_B265]
(
...
---------------------------------------------------------------------------------------------------
-- La spesa ammissibile è superiore ad euro  100€
...
IF @COSTO_INVESTIMENTI<100 BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - L`ammontare rendicontato sulla misura 132 deve essere superiore a 100 €.'
END
	
------------------------------------------------------------------------------------------------

DECLARE @COUNT_A  INT, @COUNT_B INT,
		@COUNT_C int,@COUNT_D int, @RESULT INT = 1
SET @COUNT_A = (SELECT COUNT(*) FROM @INVESTIMENTI_TEMP AS PI 
INNER JOIN VPRIORITA_X_INVESTIMENTI PXI  ON PI.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO  
WHERE  ID_PRIORITA IN(799,864,1081) AND  PXI.DESCRIZIONE  ='2012' )
 
SET @COUNT_B = (SELECT COUNT(*) FROM @INVESTIMENTI_TEMP AS PI 
INNER JOIN VPRIORITA_X_INVESTIMENTI PXI  ON PI.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO  
WHERE  ID_PRIORITA IN(799,864,1081) AND  PXI.DESCRIZIONE  ='2013' )

SET @COUNT_C = (SELECT COUNT(*) FROM @INVESTIMENTI_TEMP AS PI 
INNER JOIN VPRIORITA_X_INVESTIMENTI PXI  ON PI.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO  
WHERE  ID_PRIORITA IN(799,864,1081) AND  PXI.DESCRIZIONE  ='2014' )
 
SET @COUNT_D = (SELECT COUNT(*) FROM @INVESTIMENTI_TEMP AS PI 
INNER JOIN VPRIORITA_X_INVESTIMENTI PXI  ON PI.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO  
WHERE  ID_PRIORITA IN(799,864,1081) AND  PXI.DESCRIZIONE  ='2015' )
...
IF (@RESULT = 0)BEGIN 
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - La rendicontazione non è completa per le 2 annualità.'
END
