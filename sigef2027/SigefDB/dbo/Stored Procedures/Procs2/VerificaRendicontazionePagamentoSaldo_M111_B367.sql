CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M111_B367]
(
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	...
	IF @COSTO_INVESTIMENTI_NOLEGGIO>(@COSTO_INVESTIMENTI*0.10) BEGIN
		...
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo noleggio o costo di reintegrazione di attrezzature supera del 10% il costo del progetto.'
	END
END
...
----------------------------------------------------------------------------------
-- 2. Le spese di coordinamento <= 5% totale domanda (Codifica investimento = 12)
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	...
	IF @COSTO_INVESTIMENTI_COORD>(@COSTO_INVESTIMENTI*0.05) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo per coordinamento supera il 5% del costo del progetto.'
	END
END
-----------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------
  -- 4 - acquisto di servizi sul mercato per una quota non superiore al 10% del costo del progetto.

DECLARE @COSTO_INVESTIMENTI_RICOG DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_RICOG=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV 
			INNER JOIN DETTAGLIO_INVESTIMENTI DI ON INV.ID_DETTAGLIO_INVESTIMENTO = DI.ID_DETTAGLIO_INVESTIMENTO  
	WHERE DI.COD_DETTAGLIO IN( '4')
  
	IF @COSTO_INVESTIMENTI_RICOG>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo per l`acquisto di servizi sul mercato supera il 10% del costo del progetto.'
	END
END
