CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B455]
(
...
SELECT @COSTO_INVESTIMENTI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO in (1,4) -- inve + fidej
----------------------------------------------------------------------------------------------------------------

IF (isnull(@COSTO_INVESTIMENTI,0) < 50000)BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo dell`investimento ammissibile all`aiuto > = € 50.000,00.'
END
