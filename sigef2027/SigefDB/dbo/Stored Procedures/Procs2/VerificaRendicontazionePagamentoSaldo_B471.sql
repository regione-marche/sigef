CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B471]
(
...
-- DESCRIZIONE	QUERY_SQL	VAL_MASSIMO
--Costo investimento ammissibile <= € 150.000,00 al netto dell`IVA
--Spese per acquisto terreni nel limite del 10% del costo totale della domanda di aiuto	
----
---------------------------------------------------------------------------------------------------
...
IF (isnull(@COSTO_INVESTIMENTI,0) > 150000)BEGIN
	...
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo investimento > di 150.000€.'
END
---------------------------------------------------------------------------------------------------
-- Spese per acquisto terreni nel limite del 10% del costo totale della domanda di aiuto	
DECLARE @COSTO_TERRENI DECIMAL(18,2)
SELECT @COSTO_TERRENI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 471 AND DI.COD_DETTAGLIO = 'a')

IF isnull(@COSTO_TERRENI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese per acquisto terreni nel limite del 10% del costo totale della domanda di aiuto.'
END
---------------------------------------------------------------------------------------------------

-- Costo per miglioramento ambientale <= 20% del costo totale del progetto	
DECLARE @COSTO_MIGL_AMBIENTALI DECIMAL(18,2)
SELECT @COSTO_MIGL_AMBIENTALI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 471 AND DI.COD_DETTAGLIO = 'd')

IF isnull(@COSTO_MIGL_AMBIENTALI,0)>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo per miglioramento ambientale <= 20% del costo totale del progetto.'
END
---------------------------------------------------------------------------------------------------

-- Costo per acquisto arredi <= 15% del costo totale del progetto	
DECLARE @COSTO_ARREDI DECIMAL(18,2)
SELECT @COSTO_ARREDI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 471 AND DI.COD_DETTAGLIO = 'e')

IF isnull(@COSTO_ARREDI,0)>(@COSTO_INVESTIMENTI*0.15) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo per acquisto arredi <= 15% del costo totale del progetto.'
END
---------------------------------------------------------------------------------------------------
