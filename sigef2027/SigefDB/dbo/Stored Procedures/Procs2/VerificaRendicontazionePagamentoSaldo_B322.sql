CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B322]
(
...
-- Spese tecniche max 10% del costo totale del progetto	
-- Lavori in economia max 20% del costo totale del progetto -- COD_SPECIFICA in ('a1','b1')
---------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------------------
-- Lavori in economia max 20% del costo totale del progetto
DECLARE @COSTI_INTERNI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_INTERNI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											...
											WHERE CI.ID_BANDO = 322 AND SPI.COD_SPECIFICA in ('a1','b1'))
					
	IF isnull(@COSTI_INTERNI,0)>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Lavori in economia max 20% del costo totale del progetto'
	END
END
----------------------------------------------------------------------------------------------------------------
-- Spese tecniche max 10% del costo totale del progetto
DECLARE @COSTO_SPESE_TECNICHE int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_SPESE_TECNICHE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 															
											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   						WHERE CI.ID_BANDO = 322 AND CI.CODICE = '3' AND DI.COD_DETTAGLIO = 'c')
					
	IF isnull(@COSTO_SPESE_TECNICHE,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese tecniche max 10% del costo totale del progetto.'
	END
END
------------------------------------------------------------------------------------------------------
/*********************************************************************************************************/
