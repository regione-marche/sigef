CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M311_B115]
(
...
DECLARE @IMPORTO_LORDO DECIMAL(18,2)
SELECT @IMPORTO_LORDO =R.VAL_NUMERICO FROM DOMANDA_PAGAMENTO_REQUISITI R WHERE ID_REQUISITO = 509 AND ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO
IF(@IMPORTO_LORDO IS NULL )BEGIN 
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Indicare il valore relativo ai ricavi lordi generabili dall`investimento.'
END
ELSE IF @RATA_INTEGRAZIONE >(ISNULL(@IMPORTO_LORDO,0)*0.4) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - La rata annua di reintegrazione supera il 40% rispetto ai ricavi lordi generabili dall`investimento.'
END
