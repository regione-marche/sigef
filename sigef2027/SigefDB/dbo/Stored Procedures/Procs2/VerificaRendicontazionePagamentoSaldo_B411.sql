CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B411]
(
...
IF ISNULL(@COSTO_INVESTIMENTI,0)>0 BEGIN
	DECLARE @COSTO_INV DECIMAL(18,2), @BB INT						
	
	SELECT @BB=COUNT(ID_PRIORITA) 
	FROM PRIORITA_X_PROGETTO PP WHERE ID_PROGETTO = @ID_PROGETTO AND ID_PRIORITA = 537
   
	IF(ISNULL(@BB, 0) >0 )BEGIN
		IF (@COSTO_INVESTIMENTI > 35000) BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Totale investimento per B&B maggiore di 35.000 €.'
		 END
	 END
	ELSE BEGIN
		IF (@COSTO_INVESTIMENTI > 90000) BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Totale investimento maggiore di 90.000 €.'
		END
	END
END
...
IF ISNULL(@COSTO_INVESTIMENTI,0)>0 BEGIN
DECLARE	@id_impresa varchar(16),
		@CONTRIBUTI decimal(18,2) , 
		@CONTRIBUTO_CORRENTE decimal(18,2),
		@CONTRIBUTO_TOTALE decimal(18,2)

SELECT @id_impresa = ID_IMPRESA FROM PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO

SELECT @CONTRIBUTI= VAL_NUMERICO FROM DOMANDA_PAGAMENTO_REQUISITI WHERE ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO AND ID_REQUISITO = 518

SELECT @CONTRIBUTO_CORRENTE = SUM(CONTRIBUTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 
		  
	IF ((@CONTRIBUTO_CORRENTE + @CONTRIBUTI) >200000) BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Non sono rispettate le disposizioni in tema “deminimis”.'
	END
END
...
SELECT @id_impresaCUAA = ID_IMPRESA FROM PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO

SELECT @CONTA= (SELECT COUNT(*) FROM GRADUATORIA_PROGETTI GP INNER JOIN vPROGETTO VSP ON GP.ID_PROGETTO = VSP.ID_PROGETTO 
				WHERE VSP.ID_IMPRESA  = @id_impresaCUAA  and VSP.ORDINE_FASE >= 4 and VSP.ORDINE_FASE < 90 and VSP.COD_STATO != 'N' and VSP.COD_STATO != 'E' and GP.ID_PROGETTO != @ID_PROGETTO)
  								   
	IF(ISNULL(@CONTA, 0) >0)BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il cuaa risulta finanziabile per altri bandi PSR 2007/2013 Marche - Misure strutturali (limitatamente alle risorse FEASR)'
	END
END
...
--Spese per informatica e telematica >= 5% totale investimenti
...
--BEGIN
--	SELECT @CountAsoftware=COUNT(ID_INVESTIMENTO) 
--	FROM @INVESTIMENTI_TEMP IT 
...
--	SELECT @CountBsiti=COUNT(ID_INVESTIMENTO) 
--	FROM @INVESTIMENTI_TEMP IT
--			INNER JOIN SPECIFICA_INVESTIMENTI SPI ON IT.ID_SPECIFICA_INVESTIMENTO= SPI.ID_SPECIFICA_INVESTIMENTO AND IT.ID_DETTAGLIO_INVESTIMENTO = SPI.ID_DETTAGLIO_INVESTIMENTO
--	WHERE SPI.COD_SPECIFICA = 'b'
						
--	IF (@CountBsiti > 0 AND @CountAsoftware > 0)
--	BEGIN	
--		SELECT @CostoTIC = SUM(IMPORTO_TOTALE_RENDICONTATO) 
--		FROM @INVESTIMENTI_TEMP T
--			INNER JOIN CODIFICA_INVESTIMENTO C ON C.ID_CODIFICA_INVESTIMENTO= T.ID_CODIFICA_INVESTIMENTO 
--		WHERE CODICE = '3' AND ID_BANDO = 411
		
--		IF (@CostoTIC < (@COSTO_INVESTIMENTI * 0.05)) BEGIN
--			SET @TIPO_RISULTATO='N'
--			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese per informatica e telematica minori del 5% totale investimento.'
--		END
--	END
--END
/*********************************************************************************************************/
