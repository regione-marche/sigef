/*
 ...
 Il token XML di autenticazione sarà memorizzato nella variabile di sessione "COHESION_TOKEN" come stringa.
 L'XML con i dati di sessione sarà memorizzato nella variabile di sessione "COHESION_AUTH" come stringa.
    ...
    public class CohesionSSO
    {
        ...
        public void ValidateFE()
        {
            ...
            if (request.Params["auth"] == null)
            {
                ...
                if (!string.IsNullOrEmpty(request.Url.Query)) urlValidate = urlValidate.Replace(request.Url.Query, "");
                string urlRichiesta = request.Url.Scheme + "://" + request.Url.Host + ((request.Url.Port) == 80 ? "" : ":" + request.Url.Port) + request.Params["ReturnUrl"];
                string auth = "<dsAuth xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns=\"http://tempuri.org/Auth.xsd\"><auth><user /><id_sa /><id_sito>" + idSito + "</id_sito><esito_auth_sa /><id_sessione_sa /><id_sessione_aspnet_sa /><url_validate>" + urlValidate.Replace("&", "&amp;").Replace("http", "https") + "</url_validate><url_richiesta>" + urlRichiesta.Replace("&", "&amp;").Replace("http", "https") + "</url_richiesta><esito_auth_sso /><id_sessione_sso /><id_sessione_aspnet_sso /><stilesheet>" + this.additionalData + "</stilesheet></auth></dsAuth>";
            ...
            else
            {
                ...
                if (esitoAuthSSO.Equals("OK"))
                {
                    ...
                    if (!token.Equals("") && token.IndexOf("<AUTH>NO</AUTH>") == -1)
                    {
                        ...
                        int loginStart = token.IndexOf("<codice_fiscale>") + 16;
                        int loginStop = token.IndexOf("</codice_fiscale>");
                        ...
                        session["COHESION_TOKEN"] = token;
                        session["COHESION_AUTH"] = auth;
        ...
        {
            if (session["COHESION_AUTH"] != null)
            {
                System.Xml.XmlDocument authXml = new System.Xml.XmlDocument();
                authXml.LoadXml(session["COHESION_AUTH"].ToString());
                String idsessioneSSO = authXml.GetElementsByTagName("id_sessione_sso")[0].InnerText;
                String idsessioneSSOASPNET = authXml.GetElementsByTagName("id_sessione_aspnet_sso")[0].InnerText;
