...
{
    public partial class PianoInvestimentiIntervento : SiarLibrary.Web.SigefUserControl
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            if (Progetto != null)
            {
                ...
                if (nascodiDiv != "True")
                {
                    SiarBLL.VpianoInvestimentiInterventoCollectionProvider inter_prov = new SiarBLL.VpianoInvestimentiInterventoCollectionProvider();
                    SiarLibrary.VpianoInvestimentiInterventoCollection inter_coll = new VpianoInvestimentiInterventoCollection();
                    ...
                    else
                    {
                        if (CodiceFase == "PDOMANDA" || CodiceFase == "IDOMANDA")
                            inter_coll = inter_prov.GetPianoInvestimentiIntervento(Progetto.IdProgetto, CodiceFase , id_intervento);
                        else
                            inter_coll = inter_prov.GetPianoInvestimentiInterventoVariante(Progetto.IdProgetto, CodiceFase, Variante.IdVariante, id_intervento);
                        if (inter_coll.Count == 0)
                            dBoxIntervento.Visible = false;
                        else
                        {                            
                            dBoxIntervento.Visible = true;
                            dgInterventi.DataSource = inter_coll;
        ...
        void dgInterventi_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                else
                {
                    ...
                    string descrizione = "Codifica: <b>" + inv.CodificaInvestimento + "</b><br />Dettaglio: <b>" + inv.DettaglioInvestimenti
                    ...
                    e.Item.Cells[4].Text = "<a href='InvestimentiFondoPerduto.aspx?idpcurrent=" + pr.IdProgetto.ToString() + "&idinv=" + pr.Id.ToString() + "' >" +
                        "<img src='"+pathImages+"' style='width:24px;height:24px;cursor:pointer' alt='Visualizza investimento'/></a>";
