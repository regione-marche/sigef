...
namespace web.CONTROLS
{
    public partial class ZoomLoader : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            _searchFunction = _abilitaModifica ? "mostraPopupTemplateZoom('" + tbZoomMain.UniqueID.Replace('$', '_') + "','divBKGMessaggio');"
        ...
        private void CaricaTableParametri()
        {
            ...
            if (!string.IsNullOrEmpty(_keySearch))
            {
                ...
                div.Attributes.Add("class", "row bd-form pt-4");                
                ...
                foreach (string parametro in _keySearch.Split('|'))
                {
                    ...
                    divCol.Attributes.Add("class", "col-sm-12 form-group");
                    ...
                    if (tipo_parametro == "h")
                    {
                        ...
                        divCol.Controls.Add(hdn);
                        div.Controls.Add(divCol);
                    }
                    else
                    {
                        //testo parametro
                        ...
                        switch (tipo_parametro)
                        {
                            case "i":
                                ...
                                divCol.Controls.Add(ibox);
                                break;
                            case "d":
                                SiarLibrary.Web.DateTextBox dbox = new SiarLibrary.Web.DateTextBox();
                                dbox.ID = "txt" + id_controllo;
                                dbox.Label = dati_parametro[0] + ":";
                                dbox.AddInnerAttribute("keyValueSearchGroup", this.ID);
                                dbox.AddInnerAttribute("keyValueSearch", chiave_parametro);
                                divCol.Controls.Add(dbox);
                                divCol.Controls.Add(new LiteralControl("(click con il destro per il calendario)"));
                                break;
                            case "da":
                                SiarLibrary.Web.DateTextBoxAgid dboxagid = new SiarLibrary.Web.DateTextBoxAgid();
                                dboxagid.ID = "txt" + id_controllo;
                                dboxagid.Label = dati_parametro[0] + ":";
                                dboxagid.AddInnerAttribute("keyValueSearchGroup", this.ID);
                                dboxagid.AddInnerAttribute("keyValueSearch", chiave_parametro);
                                divCol.Controls.Add(dboxagid);                                
                                break;
                            //case "s":
                            ...
                            case "lst":
                                ...
                                combo.GetType().GetProperty("Label").SetValue(combo, dati_parametro[0] + ":");
