    ...
    public partial class PianoInvestimenti : SiarLibrary.Web.SigefUserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Progetto != null)
            {
                ...
                //if (id_da_selezionata == 0) tbMisura[0].Cells[0].Attributes.Add("class", "tdMisura selected");

                HtmlGenericControl liDefault = new HtmlGenericControl("li");
                liDefault.Attributes.Add("class", "nav-item");

                HtmlGenericControl aDefault = new HtmlGenericControl("a");

                if (id_da_selezionata == 0)
                {
                    aDefault.Attributes.Add("class", "nav-link active");
                }                
                else
                {
                    aDefault.Attributes.Add("class", "nav-link");                    
                }
                aDefault.Attributes.Add("onclick", "mostraMisura(0);");
                aDefault.InnerText = "VISUALIZZA TUTTI";
                liDefault.Controls.Add(aDefault);
                tbMisura.Controls.Add(liDefault);
                ...
                foreach (SiarLibrary.BandoProgrammazione da in interventi_coll)
                {
                    ...
                    a.Attributes.Add("onclick", "mostraMisura(" + da.IdProgrammazione + ");");

                    SiarLibrary.Zprogrammazione intervento = new SiarBLL.ZprogrammazioneCollectionProvider().GetById(da.IdProgrammazione);

                    a.InnerText = "INTERVENTO " + intervento.Codice;
                    li.Controls.Add(a);

                    tbMisura.Controls.Add(li);

                    //string css_class = "nav-item";
                    //if (id_da_selezionata > 0 && id_da_selezionata == da.IdProgrammazione.Value)
                    //{
