...
$(document).keypress(function (e) {
    ...
    if (keyCode && keyCode == 13) {
        e.preventDefault();
        let pageForm = document.getElementById("aspnetForm");
        pageForm.submit();
    }
});
...
function mostraPopupTemplateZoom(id_popup, id_bg_popup) {
    //if (!divBackground) {
    //    divBackground = document.createElement('div');
    //    divBackground.className = 'divBackGround';
    //    divBackground.setAttribute('id', 'divBKGMessaggio');
    //    document.body.appendChild(divBackground);
    //}
    var pag_form = $('#' + id_popup);
    //$('#' + id_bg_popup).show();
    $(pag_form).css({ 'position': 'absolute' });
    $(pag_form).insertAfter($('#footer-bar'));    
    pag_form.show();    
...
function mostraPopupTemplate(id_popup, id_bg_popup) {
    getDimensioniVisibili();
    var pag_form = $('#' + id_popup);  
    var left = scrollLeft + ((clientWidth - pag_form[0].offsetWidth) / 2);
    var top = scrollTop + ((clientHeight - pag_form[0].offsetHeight) / 2);
    pag_form.show().css({
...
function loadMenuAgid(codFunzione) {
    ajaxComplete = false;
    window.setTimeout("if(!ajaxComplete)$('#divLoading').show('fast');", 200);
    var funzione_pagina = $('[id$=hdnFunzioneMenuPagina]').val();
    $.post(getBaseUrl() + 'controls/menuagid.aspx', {
        funcpagina: funzione_pagina,
        funcmenu: codFunzione
    }, function (msg) {
        ajaxComplete = true;
...
function loadMegaMenuAgid(codFunzione) {
    ajaxComplete = false;
    var funzione_pagina = $('[id$=hdnFunzioneMenuPagina]').val();
    $.post(getBaseUrl() + 'controls/megamenuagid.aspx', {
        funcpagina: funzione_pagina,
        funcmenu: codFunzione
    }, function (msg) {
        ajaxComplete = true;
        $('#divMegaMenu').html(msg);
    }, "html");
}

function loadBreadcrumb(codFunzione) {
    ajaxComplete = false;
    var funzione_pagina = $('[id$=hdnFunzioneMenuPagina]').val();
    $.post(getBaseUrl() + 'controls/breadcrumbagid.aspx', {
        funcpagina: funzione_pagina,
        funcmenu: codFunzione
    }, function (msg) {
        ajaxComplete = true;
        $('#breadcrumbContainer').html(msg);
    }, "html");
}
...
function ctrlPIEstera(sz_Codice) {
    if (sz_Codice.length < 9) return false;
    var ctrlPE = sz_Codice.substr(0, 3);
    if (ctrlPE == 'DE ' || ctrlPE == 'GB ' || ctrlPE == 'FR ' || ctrlPE == 'BE ' || ctrlPE == 'AT ' || ctrlPE == 'NL ' || ctrlPE == 'DK ' || ctrlPE == 'ES ' || ctrlPE == 'CH ' || ctrlPE == 'IE ' || ctrlPE == 'CZ ' || ctrlPE == 'PL ' || ctrlPE == 'HU ' || ctrlPE == 'BE ' || ctrlPE == 'MT ' || ctrlPE == 'RO ') return true;
}
...
function dgCheckColumnAgidSelectAll(elem, _name, trclick) {
    var val = elem.checked;
    $('[id^=' + _name + '_]').each(function () {
        if (!$(this).attr('disabled')) {
            this.checked = val;
            if (trclick && this.onclick) this.onclick.apply(this);
        }
...
function SNCVZOpenDiv() {
    if (!SNCVZDivResult) {
        ...
        $(SNCVZDivResult).addClass('autocompleteContainer').css({
            'position': 'absolute',
            'width': '50%'
        });
...
function SNCZoomPlurivalore(id, search_method, select_js_method, page_index) {
    if (!SNCDivPlurivalore) {
        ...
        $(SNCDivPlurivalore).addClass('modal fade show').css(
            'display', 'block'
        ).attr({
            'role': 'dialog',
            'aria-modal': 'true',
            ...
            'tabindex': '-1',
            'aria-labelledby': 'modalZoomPlurivaloreSqlTitle'
        }).html("<div class='modal-dialog' role='document'><div class='modal-content'><div class='modal-header'><h2 class='modal-title h5' id='modalZoomPlurivaloreSqlTitle'>SELEZIONARE L'ELEMENTO DESIDERATO</h2></div><div class='modal-body'><div id='SNCTDZoomPlurivaloreResult'></div></div><div class='modal-footer'><input class='btn btn-outline-primary btn-sm py-1 m-1' type='button' value='Chiudi' onclick='$(SNCDivPlurivalore).remove(); SNCDivPlurivalore=null;'/></div></div></div></div>");
    }
...
function SNCCloseZoomPlurivalore() {
    $(SNCDivPlurivalore).remove();
    SNCDivPlurivalore = null;
...
function highlightTdMenu() {
    ...
    $('.nav-link').each(function () {
        if (this.onclick && this.onclick.toString().toLowerCase().indexOf(page_name.toLowerCase()) > 0)
        ...
        if (Number(msg.RisultatoEsecuzione == 0)) {
            ...
            if (jobj.length == 1) window.open(getBaseUrl() + 'VisualizzaDocumento.aspx');
                ...
                if (!divNPElencoDocumenti) {
                    ...
                    $(divNPElencoDocumenti).addClass('modal it-dialog').css({
                        'position': 'absolute'                        
                    });
                    $(divNPElencoDocumenti).attr("tabindex", "-1");
                    $(divNPElencoDocumenti).attr("role", "dialog");
                    $(divNPElencoDocumenti).attr("data-focus-mouse", "false");
                }
                var grid_elenco = "<table class='table table-striped'><tr><th></th><th>Titolo</th><th>Formato</th></tr>";
                for (var i = 0; i < jobj.length; i++) grid_elenco += "<tr selectable' onclick=\"window.open('" + getBaseUrl() + "VisualizzaDocumento.aspx?nf=" + i + "')\"><td style='width:100px;text-align:center;padding-left:10px;padding-right:10px;height:30px'>" + (i == 0 ? "Documento Principale" : "Allegato " + i) + "</td><td style='padding-right:10px'>" + jobj[i].Titolo + "</td><td style='width:100px;text-align:center;padding-left:10px;padding-right:10px'>" + jobj[i].Formato + "</td></tr>";
                grid_elenco += "</table>";
                ...
                $(divNPElencoDocumenti).html("<div class='modal-dialog modal-lg' role='document'><div class='modal-content'><div class='modal-header'><h2 class='modal-title h5' id='modal2Title'>SELEZIONARE IL DOCUMENTO DESIDERATO:</h2></div><div class='modal-body'>" + grid_elenco + "</div><div class='modal-footer'><input class='btn btn-outline-primary btn-sm py-1 m-1' type='button' value='Chiudi' onclick='$(divNPElencoDocumenti).remove(); divNPElencoDocumenti = null;'/></div></div></div></div>");
                //"</td></tr><tr><td style='padding:10px' align=right><input type=button class=Button value=Chiudi style='width:100px' onclick=\"$(divNPElencoDocumenti).hide();$('#divBKGMessaggio').hide();window.onscroll=null;\"/></td></tr></table>"                
        ...
        $.post(getBaseUrl() + 'CONTROLS/ajaxRequest.aspx', {
            ...
            if (Number(msg.RisultatoEsecuzione) == 0) {
                ...
                if (!divNPElencoDocumenti) {
                    divNPElencoDocumenti = document.createElement('div');
                    document.body.appendChild(divNPElencoDocumenti);
                    $(divNPElencoDocumenti).addClass('modal it-dialog-scrollable fade show');
                    $(divNPElencoDocumenti).attr('role', 'dialog');
                    $(divNPElencoDocumenti).attr('aria-modal', 'true');
                    $(divNPElencoDocumenti).attr('data-focus-mouse', 'false');
                    $(divNPElencoDocumenti).attr('role', 'dialog');
                    $(divNPElencoDocumenti).attr('tabindex', '-1');
                    $(divNPElencoDocumenti).attr('aria-labelledby', 'modal2Title');
                    $(divNPElencoDocumenti).css('display', 'block');
                }
                ...
                //$(divNPElencoDocumenti).html("<div class='modal it-dialog-scrollable' tabindex='-1' role='dialog'><div class='modal-dialog' role='document'><div class='modal-content'><div class='modal-header'><h2 class='modal-title h5' id='modal1Title'>SELEZIONARE IL DOCUMENTO DESIDERATO:</h2></div><div class='modal-body'>" + sncACVPCreateGridElenco(jobj) + "</div><div class='modal-footer'><input class='form-control' type=text disabled=disabled value='" + segnatura + "' /><input type=button class='btn btn-primary py-1 m-1' value='Copia segnatura' onclick='copiaStringaInRam('" + segnatura + "');'/><input class='btn btn-primary py-1 m-1' type=button class=Button value=Chiudi onclick='$(divNPElencoDocumenti).hide();$('#divBKGMessaggio').hide();window.onscroll=null;'/></div></div></div></div>");
                //$(divNPElencoDocumenti).html("<div class='modal-content'><div class='modal-header'><h2 class='modal-title h5' id='modal1Title'>SELEZIONARE IL DOCUMENTO DESIDERATO:</h2></div><div class='modal-body'></div><div class='modal-footer'></div></div></div>");
                $(divNPElencoDocumenti).html("<div class='modal-dialog modal-lg' role='document'><div class='modal-content'><div class='modal-header'><h2 class='modal-title h5' id='modal2Title'>SELEZIONARE IL DOCUMENTO DESIDERATO:</h2></div><div class='modal-body'>" + sncACVPCreateGridElenco(jobj) + "</div><div class='modal-footer'><input class='form-control' type='text' disabled='disabled' value='" + segnatura + "' /><input type='button' class='btn btn-primary py-1 m-1' value='Copia segnatura' onclick='copiaStringaInRam(\"" + segnatura + "\");'/><input class='btn btn-outline-primary btn-sm py-1 m-1' type='button' value='Chiudi' onclick='$(divNPElencoDocumenti).remove(); divNPElencoDocumenti = null;'/></div></div></div></div>");
                //sncFormatDivDocumenti();
                //window.onscroll = sncFormatDivDocumenti;
...
function sncACVPCreateGridElenco(jobj) {
    ...
    var grid_elenco = "<table id=tabACVPElenco class='table table-striped'><theader><tr><th></th><th>Titolo</th><th>Formato</th></tr></theader><tbody>";
    for (var i = 0; i < jobj.length; i++) grid_elenco += "<tr " + (allow_paging && i >= page_size ? "style='display:none' " : "") + "class='DataGridRow" + (i - 2 * Math.floor(i / 2) > 0 ? "Alt" : "") + " selectable' onclick=\"window.open('" + getBaseUrl() + "VisualizzaDocumento.aspx?nf=" + i +
        "')\"><td>" + (i == 0 ? "Documento Principale" : "Allegato " + i) + "</td><td>" +
        jobj[i].Titolo + "</td><td>" + jobj[i].Formato + "</td></tr>";
    return grid_elenco + (allow_paging ? "<tr id=trACVPPaginazione class=coda>" + sncACVPCreateRigaPaginazione(jobj.length, page_size, 1) + "</tr>" : "") + "</tbody></table>";
}
