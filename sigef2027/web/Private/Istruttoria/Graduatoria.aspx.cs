    ...
    public partial class Graduatoria : SiarLibrary.Web.IstruttoriaPage
    {
        ...
        void dgProgetti_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                dgi.Cells[4].Text = "<svg class='icon icon-breadcrumb icon-secondary pointer me-1' onclick=\"javascript:" + ucSiarNewZoomPunteggio.GetOpenZoomJsFunction(graduatoria_progetto.IdProgetto) + "\" aria-hidden='true'><use href='/web/bootstrap-italia/svg/sprites.svg#it-note'></use></svg>";
                dgi.Cells[9].Text = "<svg onclick=\"location='GraduatoriaDettaglio.aspx?iddom=" + graduatoria_progetto.IdProgetto + "'\" class='icon icon-breadcrumb icon-secondary pointer me-1' aria-hidden='true'><use href='/web/bootstrap-italia/svg/sprites.svg#it-pa'></use></svg>";
        ...
        void dgDecreti_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                e.Item.Cells[4].Text = "<input type=button class='btn btn-secondary py-1' value='Seleziona' onclick=\"selezionaDecreto("
        ...
        void dgDocumenti_OrgInt_itemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (dgi.ItemType != ListItemType.Header && dgi.ItemType != ListItemType.Footer)
            {
                ...
                e.Item.Cells[4].Text = "<a title ='Atto esterno' href = '" + atto.LinkEsterno + "' target='_blank'><svg class='icon icon-breadcrumb icon-secondary icon-white me-1' aria-hidden='true'><use href='/web/bootstrap-italia/svg/sprites.svg#it-search'></use></svg>Visualizza</a>";
            }
        }
        ...
        protected void btnSalvaOrdinamento_Click(object sender, EventArgs e)
        {
            ...
            string[] voci = ((SiarLibrary.Web.CheckColumnAgid)dgProgetti.Columns[13]).GetSelected();
            if (voci.Length > 0)
            {
