...
namespace web.Private.Psr.Bandi
{
    public partial class DettaglioPagamento : SiarLibrary.Web.BandoPage
    {
        ...
        protected void btnSalvaTipoPagamento_Click(object sender, EventArgs e)
        {
            try
            {
                string[] tipi_selezionati = ((SiarLibrary.Web.CheckColumnAgid)dgTipoPagamento.Columns[7]).GetSelected();
                ...
                foreach (SiarLibrary.BandoTipoPagamento tipo in tipi_pagamento)
                {
                    ...
                    {
                        throw new Exception("Per ogni tipologia di pagamento selezionata ? necessario specificare un tipo di Polizza");
                    }
                ...
                string[] requisiti_selezionati = ((SiarLibrary.Web.CheckColumnAgid)dgRequisiti.Columns[8]).GetSelected();
        ...
        protected void btnCaricaPagamento_Click(object sender, EventArgs e)
        {
            ...
            ((SiarLibrary.Web.CheckColumnAgid)dgRequisiti.Columns[4]).ClearSelected();
            ((SiarLibrary.Web.CheckColumnAgid)dgRequisiti.Columns[5]).ClearSelected();
            ((SiarLibrary.Web.CheckColumnAgid)dgRequisiti.Columns[6]).ClearSelected();
            ((SiarLibrary.Web.CheckColumnAgid)dgRequisiti.Columns[8]).ClearSelected();
        }
        ...
        void dgTipoPagamento_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType != ListItemType.Header && e.Item.ItemType != ListItemType.Footer)
            {
                ...
                e.Item.Cells[8].Text = "<input type='button' class='btn btn-primary py-1' value='Dettaglio' onclick=\"caricaPagamento('"
                    + bt.CodTipo + "')\" />";
                if (bt.IdBando != null)
                {
        ...
        protected void btnSalvaRequisitiBando_Click(object sender, EventArgs e)
        {
            ...
            { ShowError("Non ? possibile modificare la tipologia di pagamento selezionata in quanto esistono domande di pagamento presentate."); return; }
            try
            {
                ...
                foreach (string s in requisiti_selezionati)
                {
                    ...
                    if (Request.Form["chkRequisitoObbligatorio_" + s] != null)
                        obbligatorio = true;

                    if (Request.Form["chkRequisitoInserimento_" + s] != null)
                        inserimento = true;

                    if (Request.Form["chkRequisitoControllo_" + s] != null)
                        controllo = true;
                    ...
                    requisito.Obbligatorio = obbligatorio;
                    requisito.RequisitoDiControllo = controllo;
                    requisito.RequisitoDiInserimento = inserimento;
                ...
                ShowMessage("Requisiti salvati correttamente." + (salvataggio_requisiti_tralasciati > 0 ? "<br /> N.B: nr."
                    + salvataggio_requisiti_tralasciati.ToString() + " requisiti non salvati perch? associati ad altre disposizioni di misura." : ""));
            }
            catch (Exception ex) { BandoProvider.DbProviderObj.Rollback(); ShowError(ex); }
        }
            ...
            try
            {
                ...
                if (int.TryParse(hdnIdRequisito.Value, out id_requisito))
                {
                    ...
                    if (bando_requisiti_provider.Find(null, null, id_requisito).Count > 0)
                        throw new Exception("Impossibile modificare il requisito, perch? associato ad un bando.");
                }
                ...
                // Se il requisito non ? pi? plurivalore, allora elimino i relativi requisiti plurivalore
        ...
        protected void btnEliminaRequisito_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (bando_requisiti_provider.Find(null, null, id_requisito).Count > 0)
                    throw new Exception("Impossibile eliminare il requisito, perch? associato ad un bando.");
                plurivalori_provider.DeleteCollection(plurivalori_provider.Find(null, id_requisito, null));
        ...
        protected void btnAssociaValoreMultiplo_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (bando_requisiti_provider.Find(null, null, id_requisito).Count > 0)
                        throw new Exception("Impossibile salvare il valore, perch? il requisito di pagamento relativo ? associato ad un bando.");
