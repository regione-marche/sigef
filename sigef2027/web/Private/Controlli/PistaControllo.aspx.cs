...
namespace web.Private.Controlli
{
    public partial class PistaControllo : SiarLibrary.Web.PrivatePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //    {
            //        if (Request.Form["__EVENTTARGET"] == "visualizza_pista" || tabDettaglio.TabSelected == 2)
            //        {
            //            tbPistaControllo.Visible = true;
            //            int idProgetto;
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            lblNrElementi.Text = string.Format("Visualizzate {0} righe", pp.Count.ToString());
            if (pp.Count == 0)
            ...
            switch (tabDettaglio.TabSelected)
            {
                ...
                default:
                    ...
                    tbElencoBando.Visible = true;
                    tbPistaControllo.Visible = false;
                    break;
            }
        ...
        protected void rptDomandePagamento_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                //modifiche  per domande pregresse 20/11/2019
                SiarBLL.BandoConfigCollectionProvider bcc = new SiarBLL.BandoConfigCollectionProvider();

                SiarLibrary.Progetto prog = new SiarBLL.ProgettoCollectionProvider().GetById(Convert.ToInt32(hdnIdProgetto.Value));
                ...
                SiarLibrary.BandoConfigCollection bandoPregressoPDOM = bcc.Find(prog.IdBando, "PREGPDOM", "PREGRESSO", true, null);
                SiarLibrary.BandoConfigCollection bandoPregressoIDOM = bcc.Find(prog.IdBando, "PREGIDOM", "PREGRESSO", true, null);
                //SiarLibrary.BandoConfigCollection bandoPregressoRIC = bcc.Find(vpc.Idbando, "PREGRIC", "PREGRESSO", true, null);
                ...
                SiarLibrary.DecretiXDomPagEsportazioneCollection decreti_coll = new SiarBLL.DecretiXDomPagEsportazioneCollectionProvider().Find(idDomandaPag, null, null);
                SiarLibrary.DecretiXDomPagEsportazioneCollection decretoRPT = new SiarLibrary.DecretiXDomPagEsportazioneCollection();
                SiarLibrary.DomandaPagamentoLiquidazioneCollection liqMandatoRichiestoRPT = new SiarLibrary.DomandaPagamentoLiquidazioneCollection();
                SiarLibrary.DomandaPagamentoLiquidazioneCollection liqMandatoRPT = new SiarLibrary.DomandaPagamentoLiquidazioneCollection();
                for (int i = 0; i < decreti_coll.Count; i++)
                {
                    if (decreti_coll[i].IdDecreto != null)
                    {
                        ...
                        SiarLibrary.DomandaPagamentoLiquidazioneCollection Liq_coll = new SiarBLL.DomandaPagamentoLiquidazioneCollectionProvider().FindByDecreto(idDomandaPag, null, null, null, decreti_coll[i].IdDecreto);
                ...
                var revDom_coll = new SiarBLL.TestataValidazioneCollectionProvider().GetUltimaRevisioneDomandaValida(idDomandaPag);
                if (revDom_coll.Count > 0)
                {
                ...
                //SiarLibrary.DomandaDiPagamento dom_pag = new SiarBLL.DomandaDiPagamentoCollectionProvider().GetById(idDomandaPag);
                //SiarBLL.CertspDettaglioCollectionProvider cert_dett_prov = new SiarBLL.CertspDettaglioCollectionProvider();
                //SiarBLL.CertspTestaCollectionProvider cert_testa_prov = new SiarBLL.CertspTestaCollectionProvider();
                //SiarLibrary.CertspDettaglioCollection cert_dett = cert_dett_prov.FindDefinitiviSelezionatiXProgetto(dom_pag.IdProgetto);
                //SiarLibrary.CertspTesta testa = new SiarLibrary.CertspTesta();
                ...
                //foreach (SiarLibrary.CertspDettaglio csd in cert_dett)
                //{
                //    if(idDomandaPag == csd.IdDomandaPagamento)
                //    {
                //        testa = cert_testa_prov.GetById(csd.Idcertsp);
                ...
                //Certificazione Spesa
                SiarBLL.CertspTestaCollectionProvider cert_testa_prov = new SiarBLL.CertspTestaCollectionProvider();
                SiarLibrary.CertspTesta testa = cert_testa_prov.GetByIdDomandaPagamento(idDomandaPag);
                ...
                if (testa != null && testa.Definitivo == true)
                {
                    if (testa.Datasegnatura != null && testa.Datasegnatura != "")
                        lbDataCSAdg.Text = testa.Datasegnatura.ToString("dd/MM/yyyy");
                    if (testa.Segnatura != null && testa.Segnatura != "")
