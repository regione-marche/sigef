<%@ Page Language="C#" MasterPageFile="~/SigefAgidPrivate.master" AutoEventWireup="true"
    Inherits="web.Private.Controlli.ParametriRischio" CodeBehind="ParametriRischio.aspx.cs" %>

<asp:Content ID="Content1" ContentPlaceHolderID="cphContenuto" runat="Server">

    <script type="text/javascript"><!--
    function modificaParametro(id) {
        if (confirm('Attenzione! Modificare il parametro?')) {
            $('[id$=hdnIdParametro]').val(id);
            $('[id$=btnModifica]').click();
        }
    }
    function eliminaParametro(id) {
        if (confirm('Attenzione! Eliminare il parametro?')) {
            $('[id$=hdnIdParametro]').val(id);
            $('[id$=btnElimina]').click();
        }
    }
    function pulisciCampi() {
        ...
        $('[id$=txtQueryParametro_text]').val('');
        $('[id$=hdnIdParametro]').val('');
        $('[id$=chkManuale]')[0].checked = false;
    }
//--></script>
    ...
    <table class="tableNoTab" width="900">
        ...
        <tr>
            <td>
                <Siar:DataGridAgid ID="dgParametri" runat="server" AutoGenerateColumns="False" CssClass="table table-striped" PageSize="10">
