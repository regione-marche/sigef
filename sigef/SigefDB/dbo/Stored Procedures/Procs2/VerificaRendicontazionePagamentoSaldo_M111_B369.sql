CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M111_B369]
(
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_PERSONALE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV
		INNER JOIN DETTAGLIO_INVESTIMENTI D ON D.ID_DETTAGLIO_INVESTIMENTO = INV.ID_DETTAGLIO_INVESTIMENTO
	WHERE  D.COD_DETTAGLIO = '1' 
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_ATTPROP=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV
	...
	IF @COSTO_INVESTIMENTI_ATTPROP>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo delle spese di ricognizione supera il 20% del costo del progetto.'
	END
END

------------------------------------------------------------------------------

-- 3. Spese coordinamento <= 5% totale domanda (Codifica investimento = 12)
 
DECLARE @COSTO_INVESTIMENTI_SERVEST DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_SERVEST=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV
	 INNER JOIN CODIFICA_INVESTIMENTO C ON C.ID_CODIFICA_INVESTIMENTO=INV.ID_CODIFICA_INVESTIMENTO
		WHERE  C.CODICE = '12'
 
					
	IF @COSTO_INVESTIMENTI_SERVEST>(@COSTO_INVESTIMENTI*0.05) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo per le spese di coordinamento supera il 5% del costo del progetto.'
	END
END
 
-----------------------------------------------------------------------------------------
  -- 4.  Il costo delle spese generali non supera il 5% del costo del progetto

DECLARE @COSTO_INVESTIMENTI_GEN DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_GEN=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV
	 INNER JOIN CODIFICA_INVESTIMENTO C ON C.ID_CODIFICA_INVESTIMENTO=INV.ID_CODIFICA_INVESTIMENTO
		WHERE  C.CODICE = '13'
...
-----------------------------------------------------------------------------------------
  -- 5.  Spesa relativa alle Azioni informative sulla sicurezza almeno il 20% del costo totale progetto

DECLARE @COSTO_SICUREZZA DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_SICUREZZA =SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP INV 
		INNER JOIN PRIORITA_X_INVESTIMENTI PXI ON PXI.ID_INVESTIMENTO = INV.ID_INVESTIMENTO AND PXI.ID_PRIORITA = 1155
		INNER JOIN CODIFICA_INVESTIMENTO C ON C.ID_CODIFICA_INVESTIMENTO=INV.ID_CODIFICA_INVESTIMENTO
	WHERE  C.CODICE IN ('1','2','3','4','9','10')
 
 	IF @COSTO_SICUREZZA < (@COSTO_INVESTIMENTI*0.2) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo delle azioni informative sulla sicurezza non supera il 20% del costo del progetto.'
	END
END
