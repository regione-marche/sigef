CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M311_B34]
(
...
------------------------------
-- Priorità I: Investimenti strutturali realizzati con tecniche di bioedilizia (ID:27)
...
SELECT @PesoPrioritaI=ISNULL(PESO,0),@ValoreMAXPrioritaI=ISNULL(VALORE_MAX,100) FROM vSCHEDA_X_PRIORITA 
	WHERE ID_PRIORITA=27 AND ID_SCHEDA_VALUTAZIONE=@ID_SCHEDA_VALUTAZIONE
SELECT @ValorePrioritaI=CASE WHEN COUNT(VP.PUNTEGGIO)>0 THEN MAX(VP.PUNTEGGIO) ELSE 0 END FROM PRIORITA_X_PROGETTO PP 
	INNER JOIN VALORI_PRIORITA VP ON PP.ID_VALORE=VP.ID_VALORE WHERE PP.ID_PRIORITA=27 AND PP.ID_PROGETTO=@ID_PROGETTO
