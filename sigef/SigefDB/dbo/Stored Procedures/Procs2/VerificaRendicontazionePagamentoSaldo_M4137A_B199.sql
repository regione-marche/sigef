CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M4137A_B199]
(
...
---------------------------------------------------------------------------------------------------
-- Spese generali <= 5% totale investimenti
DECLARE @COSTO_INVESTIMENTI_GENERALI DECIMAL(18,2)

SELECT @COSTO_INVESTIMENTI_GENERALI= 
		(SELECT((SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) 
		 FROM @INVESTIMENTI_TEMP AS INV_TEMP 
			INNER JOIN CODIFICA_INVESTIMENTO CI on INV_TEMP.ID_CODIFICA_INVESTIMENTO = CI.ID_CODIFICA_INVESTIMENTO
		 WHERE COD_TIPO_INVESTIMENTO = 1 AND CI.CODICE = 'e')*100)
		 /
		(SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) 
		 FROM @INVESTIMENTI_TEMP AS INV_TEMP 
		 INNER JOIN CODIFICA_INVESTIMENTO CI on INV_TEMP.ID_CODIFICA_INVESTIMENTO = CI.ID_CODIFICA_INVESTIMENTO
		WHERE COD_TIPO_INVESTIMENTO = 1 AND CI.CODICE IN ('a','f','c')))

IF @COSTO_INVESTIMENTI_GENERALI > 5 BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - L`ammontare rendicontato sulla misura 4137A per le Spese generali deve essere <= 5% totale investimenti'
END

----
-- Spese di coordinamento <=5% totale spese eventi partecipativi
DECLARE @COSTO_INVESTIMENTI_COORD DECIMAL(18,2)

SELECT @COSTO_INVESTIMENTI_COORD = (SELECT((SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP AS INV_TEMP INNER JOIN 
CODIFICA_INVESTIMENTO CI on INV_TEMP.ID_CODIFICA_INVESTIMENTO = CI.ID_CODIFICA_INVESTIMENTO
WHERE 
COD_TIPO_INVESTIMENTO = 1 AND CI.CODICE = 'd')*100)
/
(SELECT SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP AS INV_TEMP INNER JOIN 
CODIFICA_INVESTIMENTO CI on INV_TEMP.ID_CODIFICA_INVESTIMENTO = CI.ID_CODIFICA_INVESTIMENTO
WHERE 
COD_TIPO_INVESTIMENTO = 1 AND CI.CODICE = 'f'))


IF @COSTO_INVESTIMENTI_COORD > 5 BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - L`ammontare rendicontato sulla misura 4137A per le Spese di coordinamento deve essere <= 5% totale spese eventi partecipativi'
END
------------------------------------------------------------------------------------------------
