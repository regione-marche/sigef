CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M132_B159]
(
...
------------------------------------------------------------------------------------------------
DECLARE @ANNI AS TABLE (ANNO INT)
DECLARE @C INT

INSERT INTO @ANNI 
SELECT DISTINCT PXI.DESCRIZIONE  
FROM @INVESTIMENTI_TEMP AS INV 
	INNER JOIN  VPRIORITA_X_INVESTIMENTI PXI  ON INV.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO  
WHERE PXI.ID_PRIORITA = 375 AND IMPORTO_TOTALE_RENDICONTATO > 0 

SELECT @C= COUNT(*) FROM @ANNI
IF( @C!= 3)BEGIN 
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - La rendicontazione non è completa per le 3 annualità.'
END
