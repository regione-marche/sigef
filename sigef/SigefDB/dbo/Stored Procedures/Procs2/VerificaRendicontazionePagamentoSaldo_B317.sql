CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B317]
(
...
--Il contributo massimo concedibile è <=  a 40.000,00 € al netto dell’ IVA come previsto al paragrafo 8 del bando per le zone C3
DECLARE @CONTRIBUTO_RICHIESTO DECIMAL(18,2), @ZonaC3 INT, @ZonaD INT

SELECT @CONTRIBUTO_RICHIESTO=SUM(CONTRIBUTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1
SELECT @ZonaC3 = ISNULL(COUNT(ID_PRIORITA),0)FROM PRIORITA_X_PROGETTO WHERE ID_PRIORITA = 1043 AND ID_VALORE = 1035 AND ID_PROGETTO = @ID_PROGETTO
SELECT @ZonaD = ISNULL(COUNT(ID_PRIORITA),0)FROM PRIORITA_X_PROGETTO WHERE ID_PRIORITA = 1043 AND ID_VALORE = 1034 AND ID_PROGETTO = @ID_PROGETTO

IF ((@CONTRIBUTO_RICHIESTO > 40000) AND (@ZonaC3 > 0) OR (@CONTRIBUTO_RICHIESTO > 60000) AND (@ZonaD > 0))BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il contributo massimo concedibile è <= a 40.000,00 € per le zone C3 e <= a 60.000,00 € per le zone D come previsto al paragrafo 8 del bando.'
END
