CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M111_B41]
(
...
---------------------------------------------------------------------------------------------------
-- La spesa ammissibile è superiore ad euro 25.000,00
...
IF @COSTO_INVESTIMENTI<25000 BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - L`ammontare rendicontato sulla misura 111 deve essere superiore a 25.000 €.'
END
...
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_PERSONALE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO 
																							...
																							WHERE CI.ID_BANDO = 41AND DI.COD_DETTAGLIO = '1')
					
	IF @COSTO_INVESTIMENTI_PERSONALE>(@COSTO_INVESTIMENTI*0.70) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo del personale interno  supera il 70% del costo del progetto.'
	END
END

 ---------------------------------------------------------------------------------------------------------

-- 2.  Il costo dell’attività propedeutica non supera il 20% del costo del progetto


DECLARE @COSTO_INVESTIMENTI_ATTPROP DECIMAL(18,2)
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_ATTPROP=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE  ID_CODIFICA_INVESTIMENTO = (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE CODICE = '11'  AND ID_BANDO = 41)
					
	IF @COSTO_INVESTIMENTI_ATTPROP>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo dell’attività propedeutica  supera il 20% del costo del progetto.'
	END
END

------------------------------------------------------------------------------

-- 3.  Il costo per servizi esterni non supera il 10% del costo del progetto
 
DECLARE @COSTO_INVESTIMENTI_SERVEST DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_SERVEST=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO 
																							FROM CODIFICA_INVESTIMENTO CI INNER JOIN 
																											DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
																							WHERE CI.ID_BANDO = 41 AND DI.COD_DETTAGLIO = '5')
...
----------------------------------------------------------------------------------
-- 4.  Il costo per coordinamento organizzativo non supera il 5% del costo del progetto
 
DECLARE @COSTO_INVESTIMENTI_COORD DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_COORD=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE  ID_CODIFICA_INVESTIMENTO = (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE CODICE = '12'  AND ID_BANDO = 41)
...
END
-----------------------------------------------------------------------------------------
  -- 5.  Il costo delle spese generali non supera il 5% del costo del progetto

DECLARE @COSTO_INVESTIMENTI_GEN DECIMAL(18,2)
 
IF @COSTO_INVESTIMENTI>0 BEGIN
	SELECT @COSTO_INVESTIMENTI_GEN=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE  ID_CODIFICA_INVESTIMENTO = (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE CODICE = '13'  AND ID_BANDO = 41)
					
	IF @COSTO_INVESTIMENTI_GEN>(@COSTO_INVESTIMENTI*0.05) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Il costo delle spese generali non supera il 5% del costo del progetto.'
	END
END
/*********************************************************************************************************/
