CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B440]
(
...
-- Spese tecniche e generali massimo 12% dell’investimento complessivo.
-- Onorari consulenti progettisti per gli investimenti materiali massimo del 10% dell`investimento.
-- Spese generali massimo 2% dell`investimento.
...
-- Spese tecniche e generali massimo 12% dell’investimento complessivo.
DECLARE @COSTI_SPESE_TECNICHE int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_SPESE_TECNICHE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											...
											WHERE CI.ID_BANDO = 440 AND SPI.COD_SPECIFICA in ('ae', 'be', 'ce', 'a3', 'b3', 'c3', 'ad', 'bd', 'cd', 'a2', 'b2', 'c2'))
					
	IF isnull(@COSTI_SPESE_TECNICHE,0)>(@COSTO_INVESTIMENTI*0.12) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese tecniche e generali massimo 12% dell’investimento complessivo.'
	END
END
---------------------------------------------------------------------------------------------------
-- Onorari consulenti progettisti per gli investimenti materiali massimo del 10% dell`investimento.
...
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_CONSULENZE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											INNER JOIN SPECIFICA_INVESTIMENTI SPI ON (SPI.ID_DETTAGLIO_INVESTIMENTO = DI.ID_DETTAGLIO_INVESTIMENTO)
											WHERE CI.ID_BANDO = 440 AND SPI.COD_SPECIFICA in ('ad', 'bd', 'cd', 'a2', 'b2', 'c2'))
					
	IF isnull(@COSTI_CONSULENZE,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Onorari consulenti progettisti per gli investimenti materiali massimo del 10% dell`investimento.'
	END
END
---------------------------------------------------------------------------------------------------
-- Spese generali massimo 2% dell`investimento.
DECLARE @COSTI_GENERALI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTI_GENERALI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											INNER JOIN SPECIFICA_INVESTIMENTI SPI ON (SPI.ID_DETTAGLIO_INVESTIMENTO = DI.ID_DETTAGLIO_INVESTIMENTO)
											WHERE CI.ID_BANDO = 440 AND SPI.COD_SPECIFICA in ('a3', 'b3', 'c3', 'ae', 'be', 'ce'))
					
	IF isnull(@COSTI_GENERALI,0)>(@COSTO_INVESTIMENTI*0.02) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese generali massimo 2% dell`investimento.'
	END
END
