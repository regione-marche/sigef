CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B404]
(
...
---- Rispetto massimale di investimento pari a 150.000€
---- PUO' aver presentato domande sul bando precedente b1 in quel caso sommo il richiesto che non può superare 150.000

DECLARE @ID_IMPRESA int, @COSTO_OLD DECIMAL(18,2)
SELECT @ID_IMPRESA = ID_IMPRESA  FROM PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO

SELECT @COSTO_OLD=COSTO_TOTALE 
FROM GRADUATORIA_PROGETTI gp
	INNER JOIN VPROGETTO p on p.ID_PROGETTO = gp.ID_PROGETTO 
WHERE ID_IMPRESA= @ID_IMPRESA AND P.ID_BANDO = 249 AND ORDINE_FASE >3 AND COD_STATO NOT IN ('N','R','E')

IF(ISNULL(@COSTO_OLD,0) >0)BEGIN 
	IF(@COSTO_OLD +@COSTO_INVESTIMENTI )>150000 BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> -  Rispetto massimale di investimento pari a 150.000€.'
	END
END

-------------------------------------------------------------------------------------------------------------------
-- Sistemazione aree verdi nel limite massimo del 20% dell`investimento per lavori/opere.

IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	DECLARE @COSTO_VERDI DECIMAL(18,2)
	DECLARE @COSTO_OPERE DECIMAL(18,2)

	SELECT @COSTO_VERDI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
							ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN 
							DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
							WHERE  CI.ID_BANDO = 404 AND DI.COD_DETTAGLIO = 'a')


	SELECT @COSTO_OPERE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
							ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN 
							DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
							WHERE CI.ID_BANDO = 404 AND DI.COD_DETTAGLIO = 'b')

	IF isnull(@COSTO_VERDI,0)>(@COSTO_OPERE*0.20) BEGIN
			SET @TIPO_RISULTATO='N'
			SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Sistemazione aree verdi nel limite massimo del 20% dell`investimento per lavori/opere.'
	END
END
----------------------------------------------------------------------------------------------------------------

-- Verifica Intervento
DECLARE @CTI DECIMAL(18,2)
SELECT @CTI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND ID_PROGRAMMAZIONE != 274

IF (isnull(@CTI,0)> 0) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Tipologia di intervento, selezionata nel piano degli investimenti, coerente con il bando di riferimento.'
END
