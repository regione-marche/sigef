CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B449]
(
...
DECLARE @COSTO_SPESE_TEC DECIMAL(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_SPESE_TEC=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI INNER JOIN 
		                                                                     ...
		                                                                     WHERE ID_BANDO = 449 AND DI.COD_DETTAGLIO = 'c')

			IF isnull(@COSTO_SPESE_TEC,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
...
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	...
	WHERE SPI.COD_SPECIFICA in ('a1','b1')
