CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B338]
(
...
-- Costo per acquisto terreni <= 10% del costo totale del progetto	
...
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 338 AND DI.COD_DETTAGLIO = 'a')

IF isnull(@COSTO_TERRENI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo per acquisto terreni supera il 10% del costo totale del progetto.'
END
----------------------------------------------------------------------------------------------------------------
-- Costo per miglioramento ambientale <= 20% del costo totale del progetto	
DECLARE @COSTO_AMBIENTALE DECIMAL(18,2)
SELECT @COSTO_AMBIENTALE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1 AND
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 338 AND DI.COD_DETTAGLIO = 'd')

IF isnull(@COSTO_AMBIENTALE,0)>(@COSTO_INVESTIMENTI*0.20) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo per il miglioramento ambientale supera il 20% del costo totale del progetto.'
END
----------------------------------------------------------------------------------------------------------------
-- Costo per acquisto arredi <= 15% del costo totale del progetto	
...
ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO  FROM CODIFICA_INVESTIMENTO CI  INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) WHERE  CI.ID_BANDO = 338 AND DI.COD_DETTAGLIO = 'e')

IF isnull(@COSTO_ARREDI,0)>(@COSTO_INVESTIMENTI*0.15) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo per l´acquisto arredi supera il 15% del costo totale del progetto.'
END
----------------------------------------------------------------------------------------------------------------
