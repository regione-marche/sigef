CREATE PROCEDURE [dbo].[zCUPGetDescrizioneRealizzAcquistoServiziFormazione]
(
	@IdProgetto INT 
)
AS
...
--IPS.RAGIONE_SOCIALE AS denom_ente_corso,
LEFT(REPLACE(IPS.RAGIONE_SOCIALE,'*',' '),100) AS denom_ente_corso,
NULL AS obiett_corso,
NULL AS mod_intervento_frequenza,
I.CUAA as partita_iva,
--CASE WHEN BRT.TESTO IS NULL  
--THEN LEFT(IPS.RAGIONE_SOCIALE,100) 
--ELSE LEFT(CAST(BRT.TESTO AS VARCHAR(MAX)),100) END AS denom_progetto,
LEFT('PROGETTO ID SIGEF ' + CONVERT(VARCHAR(10),P.ID_PROGETTO) + ' POR MARCHE FESR 2014-2020 - Azione ' + ZP.CODICE + ' ' + ZP.DESCRIZIONE, 100) AS denom_progetto,
