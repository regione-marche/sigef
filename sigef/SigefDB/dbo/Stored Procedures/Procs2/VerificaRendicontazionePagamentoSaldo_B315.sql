CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B315]
(
...
DECLARE @CONTRIBUTO_TOTALE_REND DECIMAL(18,2)
SELECT @CONTRIBUTO_TOTALE_REND=SUM(CONTRIBUTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO=1

----------------------------------------------------------------------------------------------------------------

-- Costo totale del progetto non superiore a 60.000 € al netto dell`iva.,
IF (isnull(@COSTO_INVESTIMENTI,0) > 60000)BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo totale del progetto superiore a 60.000 € al netto dell`iva.'
END
---------------------------------------------------------------------------------------------------
----===  Il contributo rispetta il massimale per tipologia di richiedente ==---
DECLARE @RISP_MAX int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	
IF(((select isnull(COUNT(ID_PRIORITA),0) from priorita_x_progetto WHERE ID_PROGETTO = @ID_PROGETTO and ID_PRIORITA = 1012) = 0 AND (@CONTRIBUTO_TOTALE_REND > 36000)) 
	 OR 
   ((select isnull(COUNT(ID_PRIORITA),0) from priorita_x_progetto WHERE ID_PROGETTO = @ID_PROGETTO and ID_PRIORITA = 1012) = 1  AND (@CONTRIBUTO_TOTALE_REND > 60000)))
	BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il contributo rispetta il massimale per tipologia di richiedente.'
	END
END
