CREATE PROCEDURE [dbo].[ZManifestazioneSegnatureFindFind]
(
...
AS
	...
	if (@lensql<3000)
	exec sp_executesql @sql,N'@IdManifestazioneequalthis INT, @CodTipoequalthis CHAR(3), @Operatoreequalthis VARCHAR(16), @Segnaturaequalthis VARCHAR(100)',@IdManifestazioneequalthis , @CodTipoequalthis , @Operatoreequalthis , @Segnaturaequalthis ;
	else
...
GO
EXECUTE sp_addextendedproperty @name = N'backup', @value = NULL, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'ZManifestazioneSegnatureFindFind';
