CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B321]
(
...
SELECT @ZonaC3 = ISNULL(COUNT(ID_PRIORITA),0)FROM PRIORITA_X_PROGETTO WHERE ID_PRIORITA = 986 AND ID_VALORE = 1007 AND ID_PROGETTO = @ID_PROGETTO

IF ((@COSTO_INVESTIMENTI > 80000) AND (@ZonaC3 > 0))BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Rispetto massimale investimento (80.000€ zona C3).'
END
