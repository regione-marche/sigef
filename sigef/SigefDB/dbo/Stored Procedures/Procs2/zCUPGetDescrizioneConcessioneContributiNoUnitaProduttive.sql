CREATE PROCEDURE [dbo].[zCUPGetDescrizioneConcessioneContributiNoUnitaProduttive]
(
	@IdProgetto INT 
)
AS
...
--LEFT(IPS.RAGIONE_SOCIALE,100) AS beneficiario,
LEFT(REPLACE(IPS.RAGIONE_SOCIALE,'*',' '),100) AS beneficiario,
I.CUAA AS partita_iva,
--LEFT(IPS.RAGIONE_SOCIALE,100) AS struttura,
LEFT(REPLACE(IPS.RAGIONE_SOCIALE,'*',' '),100) AS struttura,
--CASE WHEN BRT.TESTO IS NULL  
--THEN LEFT(IPS.RAGIONE_SOCIALE,100) 
--ELSE LEFT(CAST(BRT.TESTO AS VARCHAR(MAX)),100) END AS desc_intervento,
LEFT('PROGETTO ID SIGEF ' + CONVERT(VARCHAR(10),P.ID_PROGETTO) + ' POR MARCHE FESR 2014-2020 - Azione ' + ZP.CODICE + ' ' + ZP.DESCRIZIONE, 100) AS desc_intervento,
...
--SELECT 
--P.ID_PROGETTO as id_progetto,
--IPS.RAGIONE_SOCIALE as beneficiario,
--I.CODICE_FISCALE as partita_iva,
--IPS.RAGIONE_SOCIALE as struttura,
----LP.DUG AS tipo_indirizzo,
--IPS.RAGIONE_SOCIALE as desc_intervento,
