CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B408_433]
(
...
SELECT @COSTO_INVESTIMENTI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP WHERE COD_TIPO_INVESTIMENTO in (1) 
----------------------------------------------------------------------------------------------------------------
-- Costo Investimento ammissibile all`aiuto >= € 50.000,00 e <= € 150.000,00
IF ((isnull(@COSTO_INVESTIMENTI,0) < 50000) or (isnull(@COSTO_INVESTIMENTI,0) > 150000))BEGIN
	SET @TIPO_RISULTATO='N'
	SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo Investimento ammissibile all`aiuto >= € 50.000,00 e <= € 150.000,00'
END
