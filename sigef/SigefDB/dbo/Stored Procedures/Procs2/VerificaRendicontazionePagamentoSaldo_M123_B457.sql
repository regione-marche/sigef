CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_M123_B457]
(
...
-- IN QUESTA SCDENZA SOLO SE SONO O.P. -- ID PRIORITA:43 è SENZA OP
DECLARE @ORTOFRUTTA INT, @COSTO_ORTOFRUTTA DECIMAL(18,2), @IMPRESA_NO_OP INT, @IMPRESA_SI_OP INT
SELECT @IMPRESA_NO_OP = COUNT(ID_PRIORITA) FROM PRIORITA_X_PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO AND ID_PRIORITA = 43
SELECT @IMPRESA_SI_OP = COUNT(ID_PRIORITA) FROM PRIORITA_X_PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO AND ID_PRIORITA IN (42,44)

IF ISNULL(@IMPRESA_NO_OP,0)=0 BEGIN  
	IF(ISNULL(@IMPRESA_SI_OP,0)>0  ) BEGIN
