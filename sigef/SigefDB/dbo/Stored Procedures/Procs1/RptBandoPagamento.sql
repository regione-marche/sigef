CREATE PROCEDURE [dbo].[RptBandoPagamento] 
(
@IdProgrammazione int
)
AS
	DECLARE @ATTIVAZIONE_BANDI TABLE (ID INT)
	INSERT INTO @ATTIVAZIONE_BANDI VALUES (@IdProgrammazione)
	INSERT INTO @ATTIVAZIONE_BANDI
	SELECT ID FROM VzPROGRAMMAZIONE WHERE ID_PADRE=@IdProgrammazione AND ATTIVAZIONE_BANDI=1
	...
	FROM vBANDO AS B INNER JOIN @ATTIVAZIONE_BANDI AB ON B.ID_PROGRAMMAZIONE=AB.ID
	INNER JOIN vzPROGRAMMAZIONE BP ON AB.ID = BP.ID
