-- 
CREATE   PROCEDURE [dbo].[calcoloStep133_3]
(
@IdProgetto int,
...
SELECT ((SELECT   COUNT(*) AS A  FROM  PIANO_INVESTIMENTI PI  
  		 WHERE  ID_PROGETTO = @IdProgetto AND ((ID_INVESTIMENTO_ORIGINALE  IS NULL AND @FASE_ISTRUTTORIA=0 AND ID_VARIANTE IS NULL )OR(ID_INVESTIMENTO_ORIGINALE  IS NOT NULL AND @FASE_ISTRUTTORIA=1 AND ID_VARIANTE IS NULL)) AND cod_tipo_investimento = 1)
		 - (SELECT COUNT(*) FROM PIANO_INVESTIMENTI AS PI 
							...
							WHERE (PI.ID_PROGETTO = @IdProgetto ) AND cod_tipo_investimento = 1  AND ((ID_INVESTIMENTO_ORIGINALE  IS NULL AND @FASE_ISTRUTTORIA=0 AND ID_VARIANTE IS NULL )OR(ID_INVESTIMENTO_ORIGINALE  IS NOT NULL AND @FASE_ISTRUTTORIA=1 AND ID_VARIANTE IS NULL)) 
							AND ID_PRIORITA = 398)))
...
SET @COUNT_INIZIATIVA=( 
		...
		- ( SELECT COUNT(*) FROM PIANO_INVESTIMENTI AS PI 
							INNER JOIN PRIORITA_X_INVESTIMENTI ON PI.ID_INVESTIMENTO = PRIORITA_X_INVESTIMENTI.ID_INVESTIMENTO 
							WHERE (PI.ID_PROGETTO = @IdProgetto ) AND COD_TIPO_INVESTIMENTO = 1  AND 
						...
						(SELECT DISTINCT ISNULL( SOGG.ID_VALORE,0) ID_VALORE ,PXI.VALORE 
						 FROM PIANO_INVESTIMENTI AS PI 
							...
							LEFT JOIN ((SELECT I.ID_INVESTIMENTO, I.ID_PROGETTO, PXI.ID_PRIORITA,   PXI.ID_VALORE, PXI.SCELTO, PXI.VALORE 
										...
										WHERE ID_PROGETTO = @IdProgetto AND COD_TIPO_INVESTIMENTO =1 AND ((ID_INVESTIMENTO_ORIGINALE  IS NULL AND @FASE_ISTRUTTORIA=0 AND ID_VARIANTE IS NULL )OR(ID_INVESTIMENTO_ORIGINALE  IS NOT NULL AND @FASE_ISTRUTTORIA=1 AND ID_VARIANTE IS NULL))))  AS SOGG ON SOGG.ID_INVESTIMENTO= PI.ID_INVESTIMENTO
						 WHERE (PI.ID_PROGETTO = @IdProgetto )AND PI.COD_TIPO_INVESTIMENTO =1 
									AND ((ID_INVESTIMENTO_ORIGINALE  IS NULL AND @FASE_ISTRUTTORIA=0 AND ID_VARIANTE IS NULL )OR(ID_INVESTIMENTO_ORIGINALE  IS NOT NULL AND @FASE_ISTRUTTORIA=1 AND ID_VARIANTE IS NULL))
						) AS SOGG)
