CREATE  PROCEDURE [dbo].[calcoloRequisitoPagamento133_3]
@ID_DOMANDA_PAGAMENTO int
AS
 BEGIN
--	Spese Generali annuali rendicontate <= 15% del totale annuale rendicontato per attività di promozione/informazione/pubblicità
 ...
 FROM (SELECT ((SUM(ISNULL(SPESE_GENERALI,0)) *100 )/SUM(ISNULL( SPESE_TOTALI,1)))AS C 
		FROM (SELECT PXI.DESCRIZIONE, 
				...
				FROM @INVESTIMENTI_VIRTUALE AS PI INNER JOIN 
					VPRIORITA_X_INVESTIMENTI PXI ON PI.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO AND ID_PRIORITA IN (228,1130,1237)
					 inner join CODIFICA_INVESTIMENTO CO ON CO.ID_CODIFICA_INVESTIMENTO= PI.ID_CODIFICA_INVESTIMENTO
				WHERE  ID_PROGETTO= @ID_PROGETTO GROUP BY PXI.DESCRIZIONE, CODICE ) AS T  
