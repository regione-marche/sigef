CREATE PROCEDURE [dbo].[RptBusinessPlan_BilancioIndustriale] 
@ID_Impresa int,
@ID_Domanda int
AS
BEGIN
	...
	-- Bilanci impresa industriali dei 3 anni precedenti

    DECLARE @CUAA varchar(16), @FLAG_DEFINITIVO bit, @DATA datetime 
	DECLARE @Count int
	
	IF (@ID_Domanda IS NOT NULL)
	 BEGIN

		SELECT @ID_Impresa = ID_IMPRESA,  @FLAG_DEFINITIVO = FLAG_DEFINITIVO
		FROM PROGETTO p WHERE P.ID_PROGETTO = @ID_Domanda 
	 
	END

	IF (@FLAG_DEFINITIVO = 1) set @DATA = (SELECT DATA FROM PROGETTO_STORICO WHERE ID_PROGETTO = @ID_Domanda and COD_STATO = 'L')
	ELSE SET @DATA = GETDATE()

	SET @Count = (SELECT COUNT(ID_IMPRESA) FROM BILANCIO_INDUSTRIALE WHERE ID_IMPRESA = @ID_Impresa   AND data_Modifica <=@DATA)
	...
	BEGIN
		...
		WHERE PREVISIONALE = 0 AND ID_IMPRESA = @ID_Impresa AND DATA_MODIFICA < @DATA ORDER BY ANNO DESC
