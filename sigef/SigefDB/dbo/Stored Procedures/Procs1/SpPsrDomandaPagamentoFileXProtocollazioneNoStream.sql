...
CREATE PROCEDURE [dbo].[SpPsrDomandaPagamentoFileXProtocollazioneNoStream]
(
	@ID_DOMANDA_PAGAMENTO INT
)
AS
	--DECLARE @ID_DOMANDA_PAGAMENTO INT=409
SELECT DISTINCT ID_FILE, NOME_FILE_EFFETTIVO, NOME_FILE_EFFETTIVO AS NOME_FILE, TIPO, HASH_CODE FROM (
	SELECT A.ID_FILE,NOME_FILE=REPLACE(REPLACE(SUBSTRING(T.DESCRIZIONE+ISNULL(' '+A.DESCRIZIONE,''),0,100),'/',' '),'"',''),
		NOME_FILE AS NOME_FILE_EFFETTIVO, RIGHT(NOME_FILE,CHARINDEX('.',REVERSE(NOME_FILE))-1) AS TIPO,0 AS ORDINE	
	...
	SELECT DISTINCT G.ID_FILE,REPLACE(REPLACE(SUBSTRING(T.DESCRIZIONE+ISNULL(' nr '+G.NUMERO+' del '+CONVERT(CHAR(10),G.DATA,104)
		+' Fornitore '+G.CF_FORNITORE,''),0,100),'/',' '),'"',''), NOME_FILE AS NOME_FILE_EFFETTIVO, RIGHT(NOME_FILE,CHARINDEX('.',REVERSE(NOME_FILE))-1),1 AS ORDINE
	...
	WHERE ID_DOMANDA_PAGAMENTO=@ID_DOMANDA_PAGAMENTO
	UNION ALL
	SELECT S.ID_FILE,REPLACE(REPLACE(SUBSTRING(T.DESCRIZIONE+ISNULL(' '+ESTREMI,''),0,100),'/',' '),'"',''), NOME_FILE AS NOME_FILE_EFFETTIVO,
