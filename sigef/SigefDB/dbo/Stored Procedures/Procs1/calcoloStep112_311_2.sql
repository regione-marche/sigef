CREATE PROCEDURE [dbo].[calcoloStep112_311_2]
@IdProgetto int, @fase_istruttoria bit=0
AS

 
-- Controllo valido per gli STEP:

-- 1. 112 - 311 - VERIFICA PUNTEGGIO MINIMO = 0,08 (CRITERI B-I-J-K) 
--DECLARE @IdProgetto INT
--SET @IdProgetto=6119

DECLARE  @ID_BANDO_311 INT, @ID_PROG_INTEG_311 INT, @IdSchedaValutazione int
...
SELECT @IdSchedaValutazione = ID_SCHEDA_VALUTAZIONE FROM BANDO B WHERE ID_BANDO = @ID_BANDO_311
...
DECLARE @ValorePrioritaB decimal(10,4),@PunteggioPrioritaB decimal(10,4)
...
EXEC @ValorePrioritaB = calcoloPriorita112_311_1 @ID_PROG_INTEG_311 

SET @PunteggioPrioritaB = ((@ValorePrioritaB/100) * @PesoPrioritaB) / @ValoreMAXPrioritaB

--SELECT @PunteggioPrioritaB as b
-- Priorità I (112 - 311 - Investimenti strutturali realizzati con tecniche di bioedilizia) ID: 105 ---> PRIORITA LIVELLO DOMANDA
-- Priorità I (Investimenti strutturali realizzati con tecniche di bioedilizia) ID: 154 II SCADENZA e III SCADENZA CALCOLO IL RAPPORTO 
...
IF @IdSchedaValutazione in (83, 114, 280) -- Michela 09/12/2010 -- inserita anche la scheda validazione per la scadenza 2010.
BEGIN
	...
	EXEC @ValorePrioritaI =  calcoloPriorita112_311_3  @ID_PROG_INTEG_311, @fase_istruttoria
...
--SELECT @PunteggioPrioritaJ AS PUNTEGGIO_J
...
--SELECT @PunteggioPrioritaK AS PUNTEGGIO_k
