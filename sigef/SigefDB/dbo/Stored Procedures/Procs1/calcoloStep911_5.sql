CREATE PROCEDURE [dbo].[calcoloStep911_5]
(
	@IdProgetto int,
...
ELSE BEGIN 
 ...
 SELECT @DATA_STO = P.DATA_FINE FROM (SELECT MAX(PCS.ID_RIF) ID_PS_MAX FROM PIANO_COLTURALE_STORICO PCS INNER JOIN PIANO_COLTURALE_DETTAGLIO PC ON PC.ID_RIF = PCS.ID_RIF WHERE PCS.ID_IMPRESA=@ID_IMPRESA AND PC.ID_CATASTO IS NOT NULL AND (DATA_INIZIO <@DATA_ULTIMA_MODIFICA ))PS INNER JOIN PIANO_COLTURALE_STORICO P ON P.ID_RIF =PS.ID_PS_MAX 
 IF(@DATA_STO IS NULL)SET @RESULT =0 
