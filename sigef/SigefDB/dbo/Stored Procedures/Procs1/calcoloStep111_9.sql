CREATE PROCEDURE [dbo].[calcoloStep111_9]
@IdProgetto int,
...
-- 14 Partecipazione di gruppi di aziende agricole ad eventi, iniziative fieristiche, espositive ed informative sulle tematiche della misura
-- 	1.500
DECLARE @SpeseIniziativeTematiche decimal (10,2)
DECLARE @NumeroIniziativeTematiche int

IF @ID_BANDO = 489 BEGIN
		SET @SpeseIniziativeTematiche = ( SELECT ISNULL(SUM(PI.COSTO_INVESTIMENTO),0)
						FROM PIANO_INVESTIMENTI PI INNER JOIN PROGETTO P ON (PI.ID_PROGETTO = P.ID_PROGETTO)
						WHERE (P.ID_PROGETTO=@IdProgetto OR P.ID_PROG_INTEGRATO=@IdProgetto) AND
						PI.COD_TIPO_INVESTIMENTO = 1 AND  ((@FASE_ISTRUTTORIA=0 AND PI.ID_INVESTIMENTO_ORIGINALE IS NULL)OR(@FASE_ISTRUTTORIA=1 AND PI.ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL)) 
						AND PI.ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO 
		...
		-- Priorita 1276
		SET @NumeroIniziativeTematiche = (SELECT ISNULL( PP.VALORE ,0)
										   FROM PRIORITA_X_PROGETTO PP  
										   WHERE PP.ID_PROGETTO = @IdProgetto AND PP.ID_PRIORITA = 1276)

		IF (@SpeseIniziativeTematiche > (1500 * ISNULL(@NumeroIniziativeTematiche,0))) SET @Result = 0 
