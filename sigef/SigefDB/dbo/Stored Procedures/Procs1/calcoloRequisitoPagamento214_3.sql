...
CREATE  PROCEDURE [dbo].[calcoloRequisitoPagamento214_3]
...
IF ((SELECT ID_IMPRESA FROM PROGETTO WHERE ID_PROGETTO = @IdProgetto) = 2447)
	...
	SET @COSTO_BENI_STRUMENTALI = (SELECT SUM( ISNULL(IMPORTO_AMMESSO, PR.IMPORTO_RICHIESTO) )
									...
									WHERE D.COD_DETTAGLIO ='06' AND ID_DOMANDA_PAGAMENTO =@ID_DOMANDA_PAGAMENTO )																			

    IF(ISNULL(@COSTO_BENI_STRUMENTALI,0) > (ISNULL(@COSTO_TOTALE,0)* 0.05) )SET @RESULT = 0 
	END
 ELSE SET @RESULT = 0 
