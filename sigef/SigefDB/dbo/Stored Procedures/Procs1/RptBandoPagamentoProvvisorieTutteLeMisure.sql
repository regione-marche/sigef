CREATE PROCEDURE [dbo].[RptBandoPagamentoProvvisorieTutteLeMisure] 
...
FROM         vDOMANDA_DI_PAGAMENTO AS DP INNER JOIN
                      vPROGETTO AS P ON DP.ID_PROGETTO = ISNULL(P.ID_PROG_INTEGRATO, P.ID_PROGETTO)
...
AND p.COD_STATO NOT IN ( 'R', 'E')) > 0 
	SELECT     ISNULL(P.ID_PROG_INTEGRATO, P.ID_PROGETTO) AS ID_PROGETTO, DP.DESCRIZIONE AS TIPO_PAGAMENTO, I.CUAA, 
                      PR.COD_PSR AS PSR_DESCRIZIONE, PR.CODICE AS MISURA, 
	...
	(CASE WHEN DISPOSIZIONI_ATTUATIVE = 1 THEN PROG_PRINC.BANDO ELSE vba.DESCRIZIONE END) AS BANDO,
	ISNULL(convert(VARCHAR(16), (CASE WHEN DISPOSIZIONI_ATTUATIVE = 1 THEN (SELECT DATA_SCADENZA 
		FROM VBANDO WHERE ID_BANDO = (SELECT TOP(1) ID_BANDO FROM vBANDO_PROGRAMMAZIONE 
		WHERE ID_DISPOSIZIONI_ATTUATIVE = vba.ID_BANDO)) ELSE DATA_SCADENZA END), 103), '') AS DATA_SCADENZA,
