...
CREATE PROCEDURE [dbo].[SpCertspGetRiepilogoSpeseXAsse_20171215]
(
	@COD_PSR VARCHAR(20),
...
(
	...
	--SELECT B.ID_PROGRAMMAZIONE, ISNULL(SUM(Q1.CRICHIESTO), 0) AS CONTRIBUTO_RICHIESTO_TOTALE, ISNULL(SUM(Q2.CAMMESSO), 0) AS CONTRIBUTO_AMMESSO_TOTALE,
	--	ISNULL(SUM(Q2.CLIQUIDATO), 0) AS CONTRIBUTO_PAGATO_TOTALE
	SELECT B.ID_PROGRAMMAZIONE, ISNULL(SUM(0), 0) AS CONTRIBUTO_RICHIESTO_TOTALE, ISNULL(SUM(0), 0) AS CONTRIBUTO_AMMESSO_TOTALE,
		ISNULL(SUM(0), 0) AS CONTRIBUTO_PAGATO_TOTALE
	FROM DOMANDA_DI_PAGAMENTO D
	INNER JOIN (
		SELECT ID_DOMANDA_PAGAMENTO,ISNULL(SUM(CONTRIBUTO_RICHIESTO), 0) AS CRICHIESTO FROM PAGAMENTI_RICHIESTI GROUP BY ID_DOMANDA_PAGAMENTO
		) Q1 ON D.ID_DOMANDA_PAGAMENTO=Q1.ID_DOMANDA_PAGAMENTO
	LEFT JOIN (
		SELECT ID_DOMANDA_PAGAMENTO,ISNULL(SUM(IMPORTO_AMMESSO), 0) AS CAMMESSO,ISNULL(SUM(IMPORTO_LIQUIDATO), 0) AS CLIQUIDATO 
	...
	WHERE SEGNATURA IS NOT NULL AND ANNULLATA=0 AND (APPROVATA IS NULL OR APPROVATA=1) AND (@COD_TIPO_PAGAMENTO IS NULL OR D.COD_TIPO = @COD_TIPO_PAGAMENTO)
	AND (@DATA_INIZIO IS NULL OR D.DATA_APPROVAZIONE > @DATA_INIZIO) AND (@DATA_FINE IS NULL OR D.DATA_APPROVAZIONE < @DATA_FINE)  
	GROUP BY B.ID_PROGRAMMAZIONE
)

SELECT X.*, y.CONTRIBUTO_RICHIESTO_TOTALE, y.CONTRIBUTO_AMMESSO_TOTALE, y.CONTRIBUTO_PAGATO_TOTALE FROM (SELECT *, CTE.TIPO + ' ' + CTE.CODICE AS ASSE FROM CTE_PROGRAMMAZIONE CTE
...
INNER JOIN vzPROGRAMMAZIONE z ON CTE.ID_PADRE = Z.ID AND CTE.ATTIVAZIONE_BANDI = 1 AND CTE.COD_PSR = @COD_PSR
LEFT OUTER JOIN DOMANDE_PRESENTATE DP ON CTE.ID = DP.ID_PROGRAMMAZIONE
