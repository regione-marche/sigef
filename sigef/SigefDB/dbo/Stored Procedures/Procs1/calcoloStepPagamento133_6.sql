CREATE  PROCEDURE [dbo].[calcoloStepPagamento133_6]
@IdProgetto int,
...
DECLARE  @ANNO VARCHAR(4), @DATA_MODIFICA DATETIME,@SOGGETTO INT,@ID_VARIANTE INT,
		@COSTO_INIZIATIVA DECIMAL(18,2), @NUM_INIZIATIVA INT, @SPESE_GENERALI DECIMAL(18,2) 
DECLARE @RESULT INT
...
(SELECT SUM(ISNULL(SPESE_TOTALI,1)) AS COSTO_TOTALE,SUM(ISNULL( SP_GENERALI,0)) AS SP_GENERALI,VALORE
 FROM (SELECT PXI.VALORE, 
		...
		FROM @INVESTIMENTI_VIRTUALE AS I 
			 INNER JOIN  PRIORITA_X_INVESTIMENTI PXI ON I.ID_INVESTIMENTO = PXI.ID_INVESTIMENTO AND ID_PRIORITA IN( 399,858)     
			 INNER JOIN CODIFICA_INVESTIMENTO CO ON CO.ID_CODIFICA_INVESTIMENTO = I.ID_CODIFICA_INVESTIMENTO
...
BEGIN
 SET @SOGGETTO=(SELECT TOP(1) ID_VALORE 
				FROM (SELECT PXI.ID_INVESTIMENTO 
					  ...
					  WHERE ID_PROGETTO =@IdProgetto ) AS INIZ  
