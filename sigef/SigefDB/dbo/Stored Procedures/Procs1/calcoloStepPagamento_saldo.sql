CREATE  PROCEDURE [dbo].[calcoloStepPagamento_saldo]
@IdProgetto int,
...
DECLARE @COUNT INT,	@RESULT INT
	...
	--DECLARE @IdProgetto INT,@IdDomandaPagamento INT;SET @IdProgetto=250;SET @IdDomandaPagamento=257

	DECLARE @ID_VARIANTE INT,@DATA_MODIFICA_PAGAMENTO DATETIME,@COD_TIPO CHAR(3)
	SELECT @DATA_MODIFICA_PAGAMENTO=DATA_MODIFICA,@COD_TIPO=COD_TIPO FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO=@IdDomandaPagamento
	...
	DECLARE @IdProgetto_CORRELATO INT,@ID_BANDO INT,@RETURN_VALUE INT
	SET @RETURN_VALUE=0
	...
	DECLARE PROGETTI CURSOR FOR (SELECT ID_PROGETTO,ID_BANDO FROM PROGETTO WHERE (ID_PROGETTO=@IdProgetto OR ID_PROG_INTEGRATO=@IdProgetto))
	OPEN PROGETTI
	FETCH NEXT FROM PROGETTI INTO @IdProgetto_CORRELATO,@ID_BANDO 
	WHILE @@FETCH_STATUS=0 BEGIN
		IF @RETURN_VALUE=0 BEGIN
			...
			SELECT @CONTRIBUTO_TOTALE=SUM(I.CONTRIBUTO_RICHIESTO),@CONTRIBUTO_DOMANDA=SUM(P.CONTRIBUTO_AMMESSO)		
			FROM PIANO_INVESTIMENTI I LEFT JOIN PAGAMENTI_RICHIESTI P ON I.ID_INVESTIMENTO=P.ID_INVESTIMENTO AND P.ID_DOMANDA_PAGAMENTO=@IdDomandaPagamento
			WHERE ID_PROGETTO=@IdProgetto_CORRELATO AND ((@ID_VARIANTE IS NULL AND ID_VARIANTE IS NULL AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL) 
			...
			IF @IdProgetto_CORRELATO=@IdProgetto AND @COD_TIPO='SLD' BEGIN
				SET @PREMIO_CONTO_CAPITALE=ISNULL(DBO.calcoloPremioContoCapitaleSaldo(@IdDomandaPagamento,1),0)
			...
			IF(@CONTRIBUTO_TOTALE>0 AND @CONTRIBUTO_DOMANDA>0) BEGIN
				...
				ELSE BEGIN
					...
					WHERE E.ID_PROGETTO=@IdProgetto_CORRELATO AND D.ID_DOMANDA_PAGAMENTO<@IdDomandaPagamento AND D.APPROVATA=1 AND D.ANNULLATA=0
					...
					IF @ANTICIPO_EROGATO>0 AND dbo.SIAR_MAX(@IMPORTO_RECUPERO_ANTICIPO_A_SAL,@QUOTA_RECUPERO_ANTICIPO_A_SAL)>0 BEGIN
					...
					ELSE SET @CONTRIBUTO_DOMANDA=@CONTRIBUTO_DOMANDA-ISNULL(@ANTICIPO_EROGATO,0)+ISNULL(@ANTICIPO_RECUPERATO,0)
					
					--IF @CONTRIBUTO_DOMANDA<0 SET @CONTRIBUTO_DOMANDA=0
