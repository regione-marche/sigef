CREATE PROCEDURE [dbo].[calcoloStep311_3]
...
-- Priorità I (311 - Investimenti strutturali realizzati con tecniche di bioedilizia) 
-- PER IL PRIMO BANDO  ID: 27 
-- PER IL SECONDO id: 178
...
IF (@ID_BANDO = 34) -- NON VIENE RICHIAMATA PER IL BANDO 34 VIENE UTILIZZATA  LA calcoloStep311_2
BEGIN
...
ELSE
BEGIN
	SET @PesoPrioritaI = (SELECT ISNULL(PESO,0) FROM vSCHEDA_X_PRIORITA WHERE ID_PRIORITA = 178 AND ID_SCHEDA_VALUTAZIONE = @IdSchedaValutazione)
	SET @ValoreMAXPrioritaI = (SELECT ISNULL(VALORE_MAX,100) FROM vSCHEDA_X_PRIORITA WHERE ID_PRIORITA = 178 AND ID_SCHEDA_VALUTAZIONE = @IdSchedaValutazione)
	EXEC @ValorePrioritaI = calcoloPriorita311_3 @IdProgetto,@fase_istruttoria
