CREATE PROCEDURE [dbo].[RptIstruttoriaVariantePianoInvestimentiPrec]
(
...
SET NOCOUNT ON;
--
--DECLARE @ID_VARIANTE_ATTUALE int 
--DECLARE @COD_Tipo_Investimento int
--SET @ID_VARIANTE_ATTUALE = 52
...
SET @ID_Domanda =(SELECT ID_PROGETTO FROM VARIANTI WHERE ID_VARIANTE=@ID_VARIANTE_ATTUALE)

SET @ID_VARIANTE =(SELECT MAX(ID_VARIANTE) FROM VARIANTI WHERE ID_PROGETTO=@ID_Domanda AND APPROVATA=1 AND ID_VARIANTE<@ID_VARIANTE_ATTUALE)
...
BEGIN
	...
	WHERE (P.ID_PROGETTO=@ID_Domanda OR P.ID_PROG_INTEGRATO=@ID_Domanda) AND
					...
					(ORDINE_FASE<4 AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL) OR
					(ORDINE_FASE>=4  AND ((@ID_VARIANTE IS NULL AND ID_INVESTIMENTO_ORIGINALE IS NOT NULL AND ID_VARIANTE IS NULL) OR
					 ID_VARIANTE=@ID_VARIANTE))) AND I.COD_TIPO_INVESTIMENTO = @COD_Tipo_Investimento AND ISNULL(COD_VARIAZIONE,'x')!='A' -- non faccio più visualizzare quelli annullati
