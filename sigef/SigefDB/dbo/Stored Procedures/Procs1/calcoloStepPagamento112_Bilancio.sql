CREATE PROCEDURE [dbo].[calcoloStepPagamento112_Bilancio]
@IdProgetto int,
...
DECLARE @Result int, @IdImpresa INT,@ID_BILANCIO INT,@DATA_PRESENTAZIONE DATETIME,@ANNO_PRESENTAZIONE INT,
...
SET @IdImpresa = (SELECT  ID_IMPRESA  FROM PROGETTO WHERE ID_PROGETTO =@IdProgetto ) 
...
IF( @DATA_PRESENTAZIONE >= (SELECT DATA_INSERIMENTO FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO =@IdDomandaPagamento))SET @Result = 1 
ELSE
BEGIN 
	SET @ANNO_PRESENTAZIONE = YEAR(@DATA_PRESENTAZIONE)
	IF(@ANNO_PRESENTAZIONE = (SELECT YEAR(DATA_BILANCIO)+ 1 FROM BILANCIO_AGRICOLO WHERE ID_BILANCIO_AGRICOLO = @ID_BILANCIO)) SET @Result = 1
	ELSE SET @Result = 0
END
 
--select @DATA_PRESENTAZIONE,(SELECT  DATA_INSERIMENTO FROM DOMANDA_DI_PAGAMENTO WHERE ID_DOMANDA_PAGAMENTO =@IdDomandaPagamento)
		...
		ELSE 
			BEGIN
			 SET @REDDITO_OPERATIVO = ( SELECT SUM( ISNULL( PLV_RICAVI_NETTI,0) + ISNULL( PLV_RICAVI_ATTIVITA,0) + ISNULL( PLV_RIMANENZE_FINALI,0) - ISNULL( PLV_RIMANENZE_INIZIALI,0) - ISNULL( VA_COSTI_MAT_PRIME,0) -
																			...
																			FROM BILANCIO_AGRICOLO WHERE ID_BILANCIO_AGRICOLO=@ID_BILANCIO )
 			        -- select @Result,@REDDITO_OPERATIVO
 			         IF(@REDDITO_OPERATIVO)<>0
							BEGIN 
						      SET @REDDITO_NETTO_PAC = @REDDITO_OPERATIVO + (SELECT  SUM ( ISNULL(RN_PAC_RICAVI,0) - ISNULL(RN_PAC_COSTI,0) + 
								...
								ELSE BEGIN IF(@REDDITO_NETTO_PAC<> (SELECT CONVERT(DECIMAL(10,1), ISNULL(FF_MP_UTILE,0))  FROM BILANCIO_AGRICOLO WHERE ID_BILANCIO_AGRICOLO=@ID_BILANCIO))SET @Result = 0			
														END			
							END
			 ELSE 						
				 SET @Result = 0
