...
CREATE PROCEDURE [dbo].[_zImportProgettoSigfrido95_Singole]
...
update _GraduatoriaSigfrido95Singole_1 set ID_PROGETTO_SIGEF = @id_prog_sigef where ID_DOMANDA_SIGFRIDO = @id_prog_sigfrido
...
FROM 
_vGrad95Singole_1 gs where gs.ID_DOMANDA = @id_prog_sigfrido
...
,gs170m.Percen#					--QUOTA_CONTRIBUTO_RICHIESTO
,null							--TASSO_ABBUONO
...
,8								--ID_UNITA_MISURA
,1								--QUANTITA
,gs170m.INVESTIMENTO_AMMESSO	--COSTO_INVESTIMENTO
,0								--SPESE_GENERALI
...
inner join _vGrad95Singole_1 g on
...
inner join _vGrad95Singole_1 gs on
...
UTENTI U on
PXI.ID_PERSONA = U.ID_PERSONA_FISICA
inner join _GraduatoriaSigfrido95Singole_1 gs on
...
--,COUNT(*) as NR_IMPRESE
--from 
--_GraduatoriaSigfrido95Singole_1 g
--where g.ID_DOMANDA in (
--select distinct ID_DOMANDA from _GraduatoriaSigfrido95Singole_1 g1 where 
...
_GraduatoriaSigfrido100 gs inner join @table_ute utx on
