CREATE  PROCEDURE [dbo].[calcoloRequisitoPagamento133_7]
...
-- CONTROLLO PER OGNI ANNO RENDICONTATO
 ...
 FROM (SELECT ((SUM(ISNULL( SPESE_PRODOTTO,0)) *100)/   SUM(ISNULL( SPESE_TOTALI,1)))AS C
		FROM (SELECT PXI.DESCRIZIONE, 
				  ...
				  'SPESE_PRODOTTO' = CASE WHEN PI.ID_DETTAGLIO_INVESTIMENTO IN (5235,5249,5263,5277) THEN SUM(IMPORTO_TOTALE_RENDICONTATO)END  
				...
				WHERE ID_PROGETTO= @ID_PROGETTO GROUP BY PXI.DESCRIZIONE, CODICE,ID_DETTAGLIO_INVESTIMENTO ) AS T  
GROUP BY DESCRIZIONE  
HAVING ((SUM(ISNULL(SPESE_PRODOTTO,0))*100)/SUM(ISNULL(SPESE_TOTALI,1))) >15 ) AS C
