CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B304]
(
	@ID_DOMANDA_PAGAMENTO INT
)
AS

---====== SE LE DOMANDI IN GRADUATORIA NON SONO TUTTE FINANZIABILE CONTROLLI SU PUNTEGGIO ======-----
...
DECLARE @COSTO_PERS_INTERNO int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_PERS_INTERNO=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_SPECIFICA_INVESTIMENTO IN (SELECT ID_SPECIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											...
											WHERE CI.ID_BANDO = 304 AND SPI.COD_SPECIFICA in ('1')) 
					
	IF isnull(@COSTO_PERS_INTERNO,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo relativo al personale interno supera il 10% del costo totale del progetto.'
	END
END
------------------------------------------------------------------------------------------------------
-- Spese per attività di rilevazione ed acquisizione dei dati e delle informazioni territoriali max 10% del totale di progetto.
DECLARE @COSTO_RILEVAZIONI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_RILEVAZIONI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		...
		DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO) 
					   	    WHERE CI.ID_BANDO = 304 AND DI.COD_DETTAGLIO in ('11','21'))
					
	IF isnull(@COSTO_RILEVAZIONI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Il costo relativo alle attività di rilevazione ed acquisizione dei dati e delle informazioni territoriali supera il 10% del costo totale del progetto.'
	END
END
