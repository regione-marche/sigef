CREATE  PROCEDURE [dbo].[calcoloPrioritaRequisitoPagamento311_B_1]
(
...
SET @TipoAreaMax = ( SELECT  top(1) COMUNI.TIPO_AREA FROM CATASTO_TERRENI 
						INNER JOIN COMUNI ON CATASTO_TERRENI.ID_COMUNE = COMUNI.ID_COMUNE
...
DECLARE SETTORE_PRODUTTIVO CURSOR FOR
  (
	SELECT PIANO_INVESTIMENTI.ID_SETTORE_PRODUTTIVO,  SUM(ISNULL(PR.CONTRIBUTO_RICHIESTO, PR.CONTRIBUTO_AMMESSO)) AS SPESA                     
	FROM PAGAMENTI_BENEFICIARIO AS PB
		 INNER JOIN	PAGAMENTI_RICHIESTI AS PR ON PB.ID_PAGAMENTO_RICHIESTO = PR.ID_PAGAMENTO_RICHIESTO 
		 INNER JOIN	PIANO_INVESTIMENTI ON PR.ID_INVESTIMENTO = PIANO_INVESTIMENTI.ID_INVESTIMENTO
WHERE (PR.ID_DOMANDA_PAGAMENTO IN
	  (SELECT ID_DOMANDA_PAGAMENTO
       FROM DOMANDA_DI_PAGAMENTO
       WHERE (APPROVATA = 1) AND (ID_PROGETTO = @IdProgetto) AND (ANNULLATA = 0) OR (ID_DOMANDA_PAGAMENTO = @ID_DOMANDA_PAGAMENTO)))
