CREATE PROCEDURE [dbo].[VerificaRendicontazionePagamentoSaldo_B259]
(
...
--Totale spese realizzazione di strutture di servizi alla popolazione di cui all’articolo 6 del bando <= 120.000€
--Totale spese di gestione <= 30.000€
--Spese di gestione di pari importo tra il primo e il secondo anno
--Totale spese per sistemazione esterna <= 10% totale progetto
...
---------------------------------------------------------------------------------------------------
--Totale spese realizzazione di strutture di servizi alla popolazione di cui all’articolo 6 del bando <= 120.000€
DECLARE @COSTO_STRUTTURE int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_STRUTTURE=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 259 AND CODICE = '1')

	IF (isnull(@COSTO_STRUTTURE,0)> 120000) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Totale spese realizzazione di strutture di servizi alla popolazione di cui all’articolo 6 del bando <= 120.000€'
	END
END
---------------------------------------------------------------------------------------------------
--Totale spese di gestione <= 30.000€
DECLARE @COSTO_SGENERALI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_SGENERALI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 259 AND CODICE IN ('2','3'))

	IF (isnull(@COSTO_SGENERALI,0)> 30000) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Totale spese di gestione <= 30.000€'
	END
END
---------------------------------------------------------------------------------------------------
--Spese di gestione di pari importo tra il primo e il secondo anno
DECLARE @COSTO_SGENERALI2 DECIMAL(18,2), @COSTO_SGENERALI3 DECIMAL(18,2)
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_SGENERALI2=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 259 AND CODICE ='2')

	SELECT @COSTO_SGENERALI3=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_CODIFICA_INVESTIMENTO IN (SELECT ID_CODIFICA_INVESTIMENTO FROM CODIFICA_INVESTIMENTO WHERE ID_BANDO = 259 AND CODICE ='3')

	IF (((isnull(@COSTO_SGENERALI2,0)+1) / (isnull(@COSTO_SGENERALI3,0)+1)) != 1) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Spese di gestione di pari importo tra il primo e il secondo anno'
	END
END
---------------------------------------------------------------------------------------------------
--Totale spese per sistemazione esterna <= 10% totale progetto
DECLARE @COSTO_TERRENI int
IF isnull(@COSTO_INVESTIMENTI,0)>0 BEGIN
	SELECT @COSTO_TERRENI=SUM(IMPORTO_TOTALE_RENDICONTATO) FROM @INVESTIMENTI_TEMP
		WHERE ID_DETTAGLIO_INVESTIMENTO IN (SELECT ID_DETTAGLIO_INVESTIMENTO FROM CODIFICA_INVESTIMENTO CI 
											INNER JOIN DETTAGLIO_INVESTIMENTI DI ON (CI.ID_CODIFICA_INVESTIMENTO = DI.ID_CODIFICA_INVESTIMENTO)
											WHERE CI.ID_BANDO = 259 AND DI.COD_DETTAGLIO = '4')
					
	IF isnull(@COSTO_TERRENI,0)>(@COSTO_INVESTIMENTI*0.10) BEGIN
		SET @TIPO_RISULTATO='N'
		SET @MESSAGGIO_RISULTATO=@MESSAGGIO_RISULTATO+'<BR /> - Costo per acquisto terreni <= 10% del costo totale del progetto.'
	END
END
