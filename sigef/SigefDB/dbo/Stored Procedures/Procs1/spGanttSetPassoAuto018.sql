...
-- Description:	Aggiorna i passi GANTT che prendono i valori in automatico dal SIGEF per "CONCLUSIONE istruttoria e valutazione domande pervenute"
-- Return: -1 se data = null; 0 se data = quella già inserita, 1 se data <> null e viene aggiornata
--
--   Legge data di CONCLUSIONE istruttoria SIGEF sulla base del GANTT e bando SIGEF collegati allo step @idStep
...
-- =============================================
CREATE PROCEDURE [dbo].[spGanttSetPassoAuto018]
	@idStep int 
AS
BEGIN
	SET NOCOUNT ON;
	...
	WHERE GANTT_STEPS.ID_STEP=@idStep and GANTT_STEPS.ID_TIPO_PASSO in (18)
	...
	SELECT @dataSIGEF = MAX(PS.DATA)
	FROM 
		PROGETTO_STORICO PS
		INNER JOIN  vPROGETTO P ON  PS.ID_PROGETTO = P.ID_PROGETTO
	...
	WHERE 
		PS.COD_STATO = 'A' and GANTT_STEPS.ID_STEP=@idStep 
	...
	begin
		print 'data di CONCLUSIONE istruttoria SIGEF non presente (= NULL)' 
	...
	-- La data CONCLUSIONE istruttoria esiste, ed è cambiata rispetto a quella presente nel DB
	...
	print 'DATI NON VARIATI: la data CONCLUSIONE istruttoria in SIGEF esiste, ma non è cambiata rispetto a quella memorizzata' 
	return 0
