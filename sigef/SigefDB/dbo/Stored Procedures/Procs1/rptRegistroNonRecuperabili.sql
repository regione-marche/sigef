CREATE PROCEDURE [dbo].[rptRegistroNonRecuperabili]
(
...
ISNULL(CONVERT(VARCHAR, DATA_CERTIFICAZIONE_NON_RECUPERABILITA, 103), '') AS DATA_CERTIFICAZIONE_NON_RECUPERABILITA,
...
ISNULL(CAST(ATNR.NUMERO AS VARCHAR) + '|' + CONVERT(VARCHAR, ATNR.DATA, 103) + '|' + CAST(ATNR.REGISTRO AS VARCHAR), '') AS ATTO_NON_RECUPERABILITA
...
LEFT JOIN ATTI ATNR ON REC.ID_ATTO_NON_RECUPERABILITA = ATNR.ID_ATTO
...
-- EXEC [rptRegistroNonRecuperabili] 45, null, null, null;