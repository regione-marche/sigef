--SALDO 123  PABS e PSR

CREATE  PROCEDURE [dbo].[calcoloStepPagamento123_5]
@IdProgetto int,
@IdDomandaPagamento int =NULL
AS
BEGIN
	--	Verifica totale investimenti in domanda >=  150000
...
( SELECT ISNULL( SUM(ISNULL( PB.IMPORTO_AMMESSO,0 ) +  ISNULL( PB.SPESA_TECNICA_AMMESSA,0 ) ) ,0)
								  FROM    PAGAMENTI_BENEFICIARIO PB INNER JOIN
										  PAGAMENTI_RICHIESTI PR ON PB.ID_PAGAMENTO_RICHIESTO = PR.ID_PAGAMENTO_RICHIESTO
								  WHERE ID_DOMANDA_PAGAMENTO = @IdDomandaPagamento )

IF((@IMPORTO_DOMANDA_ATTUALE + isnull(@IMPORTO_AMMESSO,0))>=150000 )
	SET @RESULT = 1
