...
CREATE FUNCTION [dbo].[calcoloContributoTotaleProgettoCorrelato] 
(
...
SET @ID_VARIANTE=13
*/
RETURN (SELECT ISNULL(CASE
						WHEN ISNULL ( B.IMPORTO_MAX ,0)= 0 AND (SELECT COUNT(*) FROM PRIORITA_X_PROGETTO WHERE ID_PROGETTO = @ID_PROGETTO AND ID_PRIORITA IN (141,144 ))>0 AND CONTRIBUTO> 200000 
						THEN 200000
						WHEN (B.IMPORTO_MAX IS NOT NULL AND B.IMPORTO_MAX < CONTRIBUTO) THEN B.IMPORTO_MAX 
						ELSE CONTRIBUTO 
					END,0)
		...
		OR (ID_VARIANTE=@ID_VARIANTE AND ISNULL(COD_VARIAZIONE,'x')!='A')))) GROUP BY I.ID_PROGETTO,ID_BANDO ) AS Q1
