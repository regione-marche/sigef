...
{
    public partial class AdminAteco2007 : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ((SiarLibrary.Web.PrivatePage)Page).RegisterClientScriptBlock(@"
                $('[id$=btnSalvaMonitoraggio]').click(function(){$('[id$=hdnsavedatimonitoraggio]').val('ON');});
            ");
            string app = hdnsavedatimonitoraggio.Value;
            ...
            if (IsPostBack)
            {
                if (_progetto != null && hdnsavedatimonitoraggio.Value == "")
                {
