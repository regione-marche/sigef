...
namespace web.Private.PDomanda
{
    ...
    public partial class ImpresaBilancioAgricolo : SiarLibrary.Web.ProgettoPage
    {
        ...
        {
            bil_Agricolo_col = bilancio_Agricolo_provider.Find(null, Progetto.IdImpresa, null, null, null, false);
                ...
                foreach (SiarLibrary.Progetto p in progettiazienda)
                {
                    if (string.IsNullOrEmpty(p.IdProgIntegrato) || p.IdProgIntegrato == p.IdProgetto)
                        if (p.IdProgetto != Progetto.IdProgetto)
                        {
                            ...
                            if (progetto_storico != null && progetto_storico.Count > 0 && progetto_storico[0].Data >= bil_Agricolo.DataModifica)
                            {
                                result = false;
        ...
        protected void btnSalva_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                #region Salvo Bilancio Agri
                if (!string.IsNullOrEmpty(hdnId.Value))
                {
                    if (!int.TryParse(hdnId.Value, out idBilancio)) throw new Exception("Attenzione! Il bilancio selezionato non è corretto.");
                ...
                else
                {
                    ...
                    bil_Agricolo.IdImpresa = Progetto.IdImpresa;
                ...
                new SiarBLL.ProgettoCollectionProvider(bilancio_Agricolo_provider.DbProviderObj).AggiornaProgetto(Progetto, Operatore.Utente.IdUtente);
            ...
            {
                bilancio_Agricolo_provider.DbProviderObj.Rollback();
                ShowError(ex);
            }
        }
        protected void btnElimina_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bilancio_Agricolo_provider.Delete(bilancio_Agricolo_provider.GetById(id_bil));
                ShowMessage("Bilancio eliminato correttamente.");
                bil_Agricolo_col.Remove(bil_Agricolo_col.CollectionGetById(id_bil));
                Nuovo_bilancio();
        ...
        protected void btnModifica_Click(object sender, EventArgs e)
        {
            ...
            {
                bil_Agricolo = bilancio_Agricolo_provider.GetById(id_bil);
