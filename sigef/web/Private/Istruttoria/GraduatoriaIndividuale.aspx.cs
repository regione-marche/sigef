...
namespace web.Private.Istruttoria
{
    public partial class GraduatoriaIndividuale : SiarLibrary.Web.IstruttoriaPage
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            {
                pulisciCampiDecreto();
                ((SiarLibrary.Web.CheckColumn)dgProgetti.Columns[11]).ClearSelected();
            }
                    ...
                    else
                    {
                        if (atti_coll.Count == 1)
                        {
                            ...
                            if (atti_coll[0].Importo < gp.ContributoTotale)
                            {
                                ...
                                if (!Organismi_intermedi)
                                    e.Item.Cells[8].Text += "<input type=button class=ButtonGrid style='width:130px' value='" + gp.AdditionalAttributes["NUMERO"] + "|"
        ...
        protected void btnDecreto_Click(object sender, EventArgs e)
        {
            try
            {
                string[] domande_selezionate = ((SiarLibrary.Web.CheckColumn)dgProgetti.Columns[11]).GetSelected();
