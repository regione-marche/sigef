...
namespace web.Private.admin
{
    public partial class MandatiImpresaUtente : SiarLibrary.Web.PrivatePage
    {
        ...
        {
            txtDataInizio.Text = DateTime.Now.ToString("dd/MM/yyyy");
        ...
        void dgMandati_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (m.RagioneSociale == null)
                {
                    ...
                    e.Item.Cells[3].Text = "<input type='Button' class='ButtonGrid' value='Scarica anagrafica impresa' onclick='scaricaDatiImpresa("
                    + m.IdImpresa + ");' />";
                }
                if (m.Attivo) e.Item.Cells[5].Text = "<input type='Button' class='ButtonGrid' value='Revoca mandato' onclick='revocaMandato(" + m.Id + ");' />";
                else
                {
