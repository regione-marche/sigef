...
namespace web.Private.CertificazioneSpesa
{
    ...
    {       
        ...
        private void GrigliaImporti_Popola()
        {
            ...
            dgDettaglioDati.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(dgDettaglioDati_ItemDataBound);
        ...
        public void GrigliaPercentuali_Popola()
        {
            ...
            dgPercentuali.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(dgPercentuali_ItemDataBound);
        ...
        void dgDettaglio_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                {
                    dgi.Cells[22].Text = string.Format("<input type=button value='Escludi' onclick='mostraDivEscludi({0});' class='ButtonGrid' style='width:90px'",dgi.Cells[0].Text);
                }
            }
        }
        ...
        protected void btnSelezionaAuto_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowError("Non è possibile cancellare un lotto di controlli in loco definitivo");
                return;
            }
            try
            {
        ...
        private void CreaLotto()
        {
            if (!string.IsNullOrEmpty(lstPsr.SelectedValue) && !string.IsNullOrEmpty(txtDataFine.Text))
            {
                ...
                try
                {
                    if (dataInizio == null)
                    {
                        hdnIdLoco.Value = tstProv.creaLoco(lstPsr.SelectedValue, dataFine, Operatore.Utente.IdUtente).ToString();
                    }
                    else
                    {
                        hdnIdLoco.Value = tstProv.creaLoco(lstPsr.SelectedValue, dataInizio, dataFine, Operatore.Utente.IdUtente).ToString();
                    }
                }
                catch (Exception ex)
                {
                    ShowError("Si è verificato un errore in fase di creazione del lotto di controllo in loco"); //, ex.ToString());
                }
            }
            hdnIdLoco.Value = null;
            hdnIdLocoDett.Value = null;
        ...
        protected void btnInserisciSegnatura_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hdnIdLoco.Value != "" && hdnIdLoco.Value != null)
                {
                    ...
                    SiarLibrary.CntrlocoTesta ct = provider.GetById(idLoco);
