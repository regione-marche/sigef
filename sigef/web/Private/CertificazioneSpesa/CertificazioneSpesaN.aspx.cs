...
namespace web.Private.CertificazioneSpesa
{
    public partial class CertificazioneSpesaN : SiarLibrary.Web.PrivatePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            sel = ((CheckColumn)dgDettaglio.Columns[16]).GetSelected();
            AbilitaOggettiLoad();
        ...
        {            
            ...
            if (int.TryParse(hdnIdCertSp.Value, out idCertSp))
            {
                ...
                hdnDataFinaleComplessivi.Value = cert_provider.getDataInizioPrimoLottoDefinitivi().ToString("dd/MM/yyyy");
                hdnDataFinale.Value = cert_provider.getDataUltimoFinale_ByDataLotto(tstSel.Datafine).ToString("dd/MM/yyyy");
            ...
            {
                detSel = detProv.getPrjBy_IdProgetto(idProgetto, tstSel.Datafine);
            }
        ...
        protected void btnCancella_Click(object sender, EventArgs e)
        {
            ...
            CancellaLotto();
            eliminaDecertificazioniLotto(idCertSp);
        }
        ...
        private void visualizzaTestaElenco()
        {
            ...
            var riepilogoPrevisioneProgettiCertificabiliList = new CertspDettaglioCollectionProvider().GetRiepilogoPrevisioneProgettiCertificabili("%");
        ...
        private void visualizzaDettaglioElenco()
        {
            ...
            for (int i = 0; i < dets.Count; i++)
            {
                ...
                if (dets.Count == 1)
                {
                    var dettagliCollection = detProv.getBy_IdProgetto(detSel.IdProgetto, tstSel.Datafine);
        ...
        private void visualizzaDettaglioDati()
        {
            ...
            DatiLotto_Titolo.Text = string.Format("Dati lotto definitivi - Periodo dal {0} al {1}",
                                                  new CertspDettaglioCollectionProvider().getDataUltimoFinale_ByDataLotto(tstSel.Datafine).Add(giorno).ToString("dd/MM/yyyy"),
        ...
        private void visualizzaTabDecertificazioni()
        {
            if (tstSel != null)
            {
                ...
                var decertificazioneCollection = certDecertificazioneCollectionProvider.GetDecertificazioniPerCertificazioneSpesa(tstSel.Idcertsp, tstSel.Definitivo);
        ...
        private void visualizzaSingolo()
        {
            ...
            sngs = detProv.getBy_IdProgetto(detSel.IdProgetto, tstSel.Datafine);
        ...
        {
            tstSel.IdFile = ufTstChecklist.IdFile;
            tstProv.Save(tstSel);
        }
        ...
        private void CreaLotto()
        {
            ...
            {
                SiarLibrary.NullTypes.DatetimeNT dataInizio = new SiarLibrary.NullTypes.DatetimeNT(txtDataInizio.Text);
                SiarLibrary.NullTypes.DatetimeNT dataFine = new SiarLibrary.NullTypes.DatetimeNT(txtDataFine.Text);
                if (!tstProv.noTemporaryLot(lstPsr.SelectedValue))
                {
                ...
                try
                {
                    if (dataInizio == null)
                    {
                        hdnIdCertSp.Value = tstProv.creaLotto(lstPsr.SelectedValue, dataFine, ddlTipo.SelectedValue, Operatore.Utente.IdUtente).ToString();
                    }
                    else
                    {
                        hdnIdCertSp.Value = tstProv.creaLotto(lstPsr.SelectedValue, dataInizio, dataFine, ddlTipo.SelectedValue, Operatore.Utente.IdUtente).ToString();
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void firmaPost(object sender, EventArgs e)
        {
            try
            {
                ...
                // chiedere adg cosa fare qui;
                p.setCorrispondente("Autorità di Certificazione", null, null, "mittente");
                ...
                string oggetto = "Certificazione del " + hdnDataLotto.Value;
