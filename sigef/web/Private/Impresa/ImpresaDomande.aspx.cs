            ...
            else if (dgi.ItemType != ListItemType.Footer)
            {
                ...
                if (nr_progetti == 1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            dgi.Cells[4].Text = "<input style='width:130px' type=button class=ButtonGrid value='Ripresenta domanda' onclick=\"location='nuovadomanda.aspx?idb=" + b.IdBando + "&idimp=" + Azienda.IdImpresa + "'\" />";
                        }
                        if (ordine_stato == 1 && ordine_fase > 3)
                            dgi.Cells[8].Text = "<input style='width:130px' type=button class=ButtonGrid value='prosegui >>' onclick=\"location='../PPagamento/GestioneLavori.aspx?iddom=" + id_progetto + "';\" />";
                    }
                    dgi.Cells[7].Text = "<input style='width:150px' type=button class=ButtonGrid value='" + messaggio_pulsante
                        + "' onclick=\"" + url + "\" />";
                ...
                else if (nr_progetti > 1)
                {
                    ...
                    foreach (SiarLibrary.Progetto pp in coll_prog)
                    {
                        ...
                        if (pp.OrdineFase != null && pp.OrdineFase > 3 && pp.OrdineStato == 1)
                            dgi.Cells[8].Text += "<div style=' height: 30px;padding-top:5px'><input style='width:130px' type=button class=ButtonGrid value='prosegui >>' onclick=\"location='../PPagamento/GestioneLavori.aspx?iddom=" + pp.IdProgetto.ToString() + "';\" /></div>";
                        else
                ...
                else
                {
                    ...
                    dgi.Cells[4].Text = "<input style='width:130px' type=button class=ButtonGrid value='" + messaggio_pulsante + "' onclick=\"location='"
                        + url + "'\" />";
                }
