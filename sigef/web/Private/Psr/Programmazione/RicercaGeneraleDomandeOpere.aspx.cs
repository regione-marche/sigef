...
{
    public partial class RicercaGeneraleDomandeOpere : SiarLibrary.Web.PrivatePage
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (IsPostBack)
            {
                SiarLibrary.ProgettoCollection pp = new SiarBLL.ProgettoCollectionProvider().RicercaGeneraleProgettiOperePubbliche(txtIdProgetto.Text, lstStatoProgetto.SelectedValue, ucZoomBando.SelectedValue,
        ...
        void dgDomande_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (p.CodStato == "P") e.Item.Cells[7].Text = "<input type=button class=ButtonGrid value='Sezione domanda' style='width:130px' onclick=\"location='../../pdomanda/datigenerali.aspx?iddom="
                    + p.IdProgetto + "'\" />";
                else if (!allegatiProtocollatiOk) e.Item.Cells[7].Text = "<input type=button class=ButtonGrid value='Protocolla allegati' style='width:130px' onclick=\"location='../../pdomanda/RiepilogoDomanda.aspx?iddom="
                    + p.IdProgetto + "'\" />";
                else e.Item.Cells[7].Text = "<input type=button class=ButtonGrid value='Istruttoria domanda' style='width:130px' onclick=\"location='../../istruttoria/checklistammissibilita.aspx?iddom="
                    + p.IdProgetto + "&idb=" + p.IdBando + "'\" />";
                if (p.OrdineFase.Between(4, 9))
                    e.Item.Cells[8].Text = "<input type=button class=ButtonGrid value='Gestione lavori' style='width:130px' onclick=\"location='../../ppagamento/gestionelavori.aspx?iddom="
                        + p.IdProgetto + "'\" />";
                e.Item.Cells[9].Text = "<input type=button class=ButtonGrid value='SISMAPP' style='width:130px' onclick=\"window.open('"+ System.Configuration.ConfigurationManager.AppSettings["SismAppUrl"] + p.IdProgetto.ToString()+"')\" '/>";
