...
namespace SiarBLL
{
    public partial class BandoCollectionProvider : IBandoProvider
    {
        ...
        public BandoCollection GetBandiUtilizzanoContrattiArtigiancassa()
        {
            BandoCollection bandiArtigiancassa = new BandoCollection();

            System.Data.IDbCommand cmd = DbProviderObj.GetCommand();
            cmd.CommandType = System.Data.CommandType.StoredProcedure;
            cmd.CommandText = "SpGetBandiUtilizzanoContrattiArtigiancassa";
            DbProviderObj.InitDatareader(cmd);
            while (DbProviderObj.DataReader.Read())
            {
                var bando = BandoDAL.GetFromDatareader(DbProviderObj);
                bandiArtigiancassa.Add(bando);
            }

            return bandiArtigiancassa;
        }
