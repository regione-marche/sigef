    ...
    public partial class CertspTestaCollectionProvider : ICertspTestaProvider
    {
        ...
        public int creaLotto(string codPsr, DateTime dataInizio, DateTime dataFine, string tipo, int operatore)
        {
            ...
            try
            {
                ...
                //      - controllo in loco superato, -- vincolo rimosso dal 02/07/2024 per pass Funzionalita' #214155 AND tcl.Esito_Testata = 'TRUE'
                //      - data sopralluogo entro la fine del lotto -- vincolo rimosso dal 02/07/2024 per pass Funzionalita' #214155 AND tcl.Data_Sopralluogo   <= @DataFine
                //      02/11/2023 Aggiunta esclusione progetti con rinuncia, revocati e esclusi per sicurezza riferimento pass Supporto #199803
                // 12/07/2024 Modificata INNER JOIN in LEFT JOIN Testata_Controlli_Loco per rilassamento controlli come scritto sopra
                strSql = @"WITH IRRE AS (
                            SELECT 
	                            ID_PROGETTO,
	                            ID_DOMANDA,
